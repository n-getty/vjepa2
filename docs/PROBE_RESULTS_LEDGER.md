...
> **⚠️ UNDER RE-EXAMINATION (2026-08-18) — this leave-one-out is PROTOCOL-CONFOUNDED.**
> `surgonly_fresh` (21.99) was probed at **TPN=12**; `noopenh_fresh` (28.92) and
> `sitlonly_fresh` (28.65) were probed at **TPN=4**. The launcher confound is worth
> **+9.4 to +10.3 IVT** (see the retraction in the "Triplet mid-run dip" entry) —
> *larger than the entire −6.9 "poison"* this table attributes to openh. The
> WITH-openh arm may be normal and the whole effect an artifact. A 3-seed TPN=4
> re-probe of `surgonly_fresh` is running (job 8763764). **Do not cite the −6.9
> openh poison, or any downstream claim resting on it, until that lands.**

| arm (fresh Meta-2B, 80ep) | openh? | IVT mAP (3-seed) | vs raw Meta 28.81 |
|---|---|---:|---|
| `surgonly_fresh` (all 15 src) | **yes** | 21.99 ⚠️ **TPN=12, void** | −6.8 (REGRESSED) |
...
> **⚠️ 2026-08-18 — THE INDEPENDENT CORROBORATION IS NOT INDEPENDENT.** Leonardo's
> bucket-attribution (cited below as a second method) reuses **this same TPN=12 probe
> head**: `v2_surgonly_fresh/.../best_epoch_*val_mean_micro_f1.pt` (F1 74.12), against a
> control head from the **TPN=4** `noopenh` run (F1 79.53). His headline "79.6 → 74.1
> (−5.5 F1) collapse" is therefore the launcher confound, not an openh effect — the same
> −5.5 F1 magnitude the protocol produces on a FROZEN SurgeNetXL encoder (78.0 → 75.5)
> and on verb/target F1 for every checkpoint re-probed. Both "methods" trace to one
> contaminated head. His per-bucket Δmargin analysis may still be internally valid as
> *symptom localization* (it found the effect DIFFUSE across all 21 buckets, with
> overlay and non-overlay buckets equal) — and a diffuse, source-agnostic signal is
> exactly what a probe-side artifact looks like.
>
> **A THIRD conclusion also rests on this number:** the "REFUTES catastrophic
> forgetting / rehearsal is not required" verdict below, which is the standing
> justification for keeping kinetics400 and pe_video out of the corpus.

**VERDICT (⚠️ UNDER RE-EXAMINATION — see the two warnings above): `openh`
(Open-H-Embodiment) is the cause of the v2 triplet regression.**
...
### `vitG384_prod_9M` trajectory — triplet IVT mAP (2026-08-16)

First production run of the openh/pe_video-dropped, 9.22M-sample recipe (256n, gb=6144,
`configs/vitg16_surg_vid_webdataset_single4/vitG384_prod_9M.yaml`). Sparse checkpoint
trajectory (e4/e9/e29 = 1.54M/3.07M/9.22M samples seen), single seed, 25 epochs,
topk_mean k=8, live-encoder probe — same protocol as §1's Aurora table. Scored via
`scripts/compute_triplet_map.py` on each checkpoint's val_probs_dump
(`configs/heads/triplet/trip_prod9M_e{4,9,29}.yaml`, jobs 8759435 + 8759850).

| ckpt | samples | IVT mAP | tool mAP | verb mAP | target mAP |
|---|---|---:|---:|---:|---:|
| prod9M_e4  | 1.54M | 29.04 | 91.50 | 78.14 | 50.07 |
| prod9M_e9  | 3.07M | 28.85 | 91.63 | 78.09 | 50.70 |
| prod9M_e29 | 9.22M | 28.45 | 89.45 | 77.68 | 53.09 |

**Flat-to-slightly-declining across the full budget (29.04 → 28.45, Δ −0.59)** — consistent
with the standing finding that triplet does not discriminate CPT budget the way GraSP does
(see the GraSP trajectory entry below, same run). Target mAP trends up (50.07 → 53.09) even
as overall IVT is flat, so budget may be moving sub-task performance without net IVT
movement — worth a look if a future probe breaks IVT out by task instead of the pooled
metric. **Single seed only — not yet ensembled.** Per `[[seed-ensembling-generalizes-to-triplet]]`,
seed-ensembling only rescues noisy/disagreeing seeds and does not reliably grow or shrink a
budget delta in a predictable direction, so a flat single-seed trend here is not strong
evidence either way without 2+ more seeds per checkpoint (cheap: reuses the existing head/data
pipeline, ~1-2h/seed on 1 capacity node).
...
### `vitG384_prod_9M` final checkpoint (e29) — SAR F1@10, 3-seed + seed-ensemble (2026-08-17)

Fills the gap flagged in the summary's Open section: prod_9M's e29 (final checkpoint, natural
completion at 9.22M samples) had GraSP+Triplet trajectory reads but no SAR/ESAD. Same
fully-controlled protocol as the ★ table above — `early_stop_patience` removed (full 20 epochs),
`VJEPA_PROBE_SEED={0,1,2}`, official TEST split, `head_idx=0`, `latest.pt`. Configs cloned via a
new `scripts/gen_prod9m_sar_configs.py` (mirrors `scripts/gen_v2latest_seeded_configs.py`'s
export-once/3-seed-probe/TEST-variant pattern) from the `fs_e189_export.yaml`/`fs_e159_s0_probe.yaml`
templates. Jobs: export 8760205 (4n, debug-scaling); head-train per seed required 2-3
walltime-cycle resubmits each on the 1h `debug` queue (8760224→8760330→8760453→8760588 for
seed0, 8760614→8760680→8760754 for seed1, 8760843→8760979→8761358 for seed2 — every resume
picked up cleanly via `resume_checkpoint: true`/`save_every_iters: 200`, confirmed via
`log_r0.csv` epoch continuity, no gaps or restarts-to-epoch-0); seed-ensemble scored via a new
one-arm copy `scripts/sar_seed_ensemble_prod9m.sh` (job 8761457, avoids re-scoring the other 4
arms already on record).

| scale | encoder | F1@10 (per-seed) | F1@10 mean±std | seed-ensemble |
|---|---|---|---:|---:|
| 2B | **CPT (prod9M_e29)** | 88.17 / 88.38 / 87.64 | **88.06 ± 0.31** | **89.76** (Δ +1.70) |
| 2B | CPT (fs_e159), for reference | 86.50 / 87.10 / 87.20 | 86.93 ± 0.38 | 89.04 |

**★ prod9M_e29 clears the backbone-saturated band this section spent most of its length
documenting.** Every other frozen 2B/1B arm in the ★ table above sits in 85.6–87.0 (all
statistical ties with each other, per-seed std 0.38-0.90); prod9M_e29's 88.06 is +1.13 over
fs_e159's own mean, outside fs_e159's ±0.38 spread — a genuine separation, not another tie.
Same head_idx/split/protocol as the reference row (verified via the aggregator's identical
`gt_full`-style path-set assertion, which hard-fails on any cache/split mismatch — it did not
fail here). Seed-ensembling grows the lead further (89.76 vs fs_e159's 89.04), consistent with
[[sar-seed-ensemble-result]]'s finding that 2B's ensemble axis widens rather than closes gaps.

**Read with the same caution as every single-checkpoint result in this ledger:** this is one
checkpoint from one production run, not a controlled ablation — prod_9M differs from fs_e159's
lineage in corpus composition (openh/pe_video dropped) AND total budget (9.22M samples vs
fs_e159's own training length), so this result cannot yet isolate *why* SAR moved here when it
didn't for fs_e159/e19/meta2b/meta1b. GraSP and Triplet did NOT show the same separation for
this checkpoint (GraSP trajectory rose with budget per the existing prod_9M entry above; Triplet
was flat-to-declining) — worth a look at whether SAR's action-segmentation objective is
uniquely sensitive to whatever prod_9M's recipe changed, or whether this is an SAR-specific
seed-luck artifact that a 4th seed would resolve.

Reproduce: `scripts/gen_prod9m_sar_configs.py` (config gen), `scripts/sar_seed_ensemble_prod9m.sh`
(scoring), raw dumps at `/flare/ModCon/ngetty/logs/sar_seed_ensemble_dumps/prod9M_e29_s{0,1,2}/`,
aggregate JSON at `/flare/ModCon/ngetty/logs/sar_seed_ensemble_scan/prod9M_e29_seed_ensemble.json`.

### `prod37M_e199` frozen + FT-last4 — F1@10 on TEST (2026-08-21/22, jobs 8774557 train, 8775762 score)

**Frozen** (3 seeds, standard `full_cached_384` protocol, `latest.pt`):

| seed0 | seed1 | seed2 | mean ± std |
|---:|---:|---:|---:|
| 86.95 | 87.85 | 87.50 | **87.43 ± 0.38** |

In line with every other frozen 2B arm in the ★ table above (85.6–87.0 backbone-saturated
band, ±1.5 over published SOTA 84.10) — another statistical tie, not a separation.

**FT last-4** (single seed, `configs/heads/sarrarp50/finetune/epoch_sweep/sarft_prod37M_e199_sweep.yaml`
— the validated 25-epoch, `early_stop_patience` REMOVED, `slim_checkpoints: true` ceiling-sweep
protocol, NOT the quick-FT template with early-stop; an earlier attempt on the wrong template
gave an implausible 83.87 and was discarded, see the config's own header). Every epoch checkpoint
(ep1–25 + `best.pt` + `latest.pt`) rescored individually on TEST via `scripts/eval_segmental_f1.py`
(job 8775762, 12-way parallel):

| epoch | 1 | 10 | 13 | 16 | 19 | **20** | 21 | 24 | 25 | latest | best (val-selected, ep14) |
|---|---|---|---|---|---|---|---|---|---|---|---|
| F1@10 | 77.99 | 88.88 | 90.24 | 90.06 | 90.10 | **90.81** | 90.38 | 90.32 | 90.33 | 90.33 | 89.55 |

**True ceiling = 90.81 F1@10 @ ep20** — the full ramp is monotonic from ep1 (77.99) through the
low-90-plateau (ep13 onward, all ≥90.0 except one dip at ep15/89.51), consistent with the
documented ~ep21-22 peak from prior CPT checkpoints (`[[sarft-ceiling-sweep-result]]`). The
`val_macro_f1`-selected `best.pt` (ep14, 89.55) undershoots the true peak by **1.26 F1@10** —
the known selection-metric gap (`[[val-loop-decomposes-the-iteration]]`) reproduces cleanly here
too: never cite `best.pt`'s F1@10 as the FT ceiling for this checkpoint, use the per-epoch
rescore's max instead.

**FT vs frozen: +3.38 F1@10 (90.81 vs 87.43 mean)** — a clean, large gain, well outside frozen's
±0.38 seed spread and consistent with `[[fine-tune-beats-frozen-readout]]`'s general finding
that frozen SAR readouts understate the encoder. Single-seed FT only; a 3-seed FT sweep would be
needed before citing seed-to-seed variance on this number, but the gap size (3.4pts) is large
enough that it is very unlikely to be single-seed noise (frozen's own single-seed spread here is
under 1pt).

Reproduce: `scripts/sarft_sweep_resume_chain.sh` (self-resubmitting training chain, job
8774557 lineage), `scripts/sarft_sweep_score_prod37m_e199.sh` (per-epoch rescore, job 8775762),
raw per-epoch JSON at `/flare/ModCon/ngetty/logs/sarft_sweep_scores/prod37M_e199_ep*_TEST.json`.
...
### `vitG384_prod_9M` trajectory — GraSP phase mAP (2026-08-16)

Same production run and checkpoints as the triplet trajectory above (§1). Cached probe,
`configs/heads/grasp/full_cached_384/grasp_phase_prod9M_e{4,9,29}_{export,probe}.yaml`
(cloned from the `fs_e159` template via `scripts/gen_prod9m_grasp_triplet_configs.py`),
3-LR-head sweep, scored `scripts/eval_grasp_map_cached.py` (jobs 8759901 export+head-train
for e4/e9, 8759939 for e29; export cache built once, reused across walltime-killed retries).

**⚠ Uses the OLD ctx3 clip builder (`grasp_phase_asformer_ctx3_seq_{train,val}.csv`), not
the fixed `official_ctx16` builder** — inherits every defect the "EVERY GraSP NUMBER IN THIS
SECTION IS BUILD-CONFOUNDED" audit above documents (label drift up to +14s, UI-shortcut
inflation, non-keyframe-centered windows). **Not** the eval-leak issue, though: this run's
pretraining corpus uses `grasp_noleak` (verified in `vitG384_prod_9M.yaml`), so GraSP's TEST
cases were never seen during CPT. Absolute numbers below should be read as inflated by the
build confound; treat the shape (does it rise with budget) as the more trustworthy signal
than the exact values, pending an `official_ctx16` re-probe.

| ckpt | samples | best-head mAP | ensemble-of-3 mAP | top-2 ensemble mAP | head-train ep (best.pt) |
|---|---|---:|---:|---:|---|
| prod9M_e4  | 1.54M | 67.63 | 69.18 | 69.34 | ep9 |
| prod9M_e9  | 3.07M | 68.42 | 71.66 | 71.09 | ep9 |
| prod9M_e29 | 9.22M | **70.98** | **73.52** | 73.22 | ep8 |
| meta2b (raw, build-matched ref) | — | 68.04 | — | — | (ledger §3) |

**Monotonic rise across the full budget** — best-head +3.35, ensemble +4.34 (e4→e29) — but
weigh this against the measured **~3 mAP single-seed noise floor** (`[[grasp-probe-noise-floor-3map]]`):
per-segment deltas (e4→e9: +0.79 best-head / +2.48 ensemble; e9→e29: +2.56 / +1.86) are each
individually within or at the edge of that floor. **Only the full e4→e29 delta clearly clears
noise.** Per-segment slope (best-head mAP per million samples) is 0.52 (e4→e9) vs 0.42
(e9→e29) — mildly decelerating but still positive; the ensemble slope drops more sharply
(1.62 → 0.30), a stronger diminishing-returns signature, though ensemble-of-3 is the noisier
of the two statistics. **Single seed only.** Whether the trend continues past 9.22M samples
is not resolved by this data — see the open item below for the follow-up (2x-budget rerun +
3-seed reprobe of e29) launched to test it directly.

### ★★★ RETRACTION (2026-08-18) — the Triplet "mid-run dip" was a probe-launcher confound

**Everything in the entry below about a checkpoint-level Triplet collapse is VOID.**
The dip tracks *which launcher ran the probe*, not which checkpoint was probed.

| | launcher | world size | iters/epoch | output dirs | Triplet arms |
|---|---|---|---|---|---|
| healthy | `scripts/triplet_seed_sweep.sh` (`TPN="${TPN:-4}"`) | **4** | **222** | `*_s<seed>` (from `.rendered/`) | prod9M e4/e9/e29 |
| collapsed | `scripts/triplet_diag_chain.sh` (`WORLD_SIZE=12`, `mpiexec -n 12`, hardcoded) | **12** | **74** | unsuffixed | prod9M e14/e19/e24, prod18M, all `v2_*` |

Same YAML, same LR — 3× the global batch and 1/3 the optimizer steps. The split is
perfect: **no checkpoint was ever measured under both protocols**, which is exactly
why it read as a property of the checkpoints. `tasks_per_node: 12` in the YAML is
inert; the launcher's `-n` decides. `triplet_seed_sweep.sh` already carries a comment
documenting this exact failure mode ("the SNX 'cross-platform offset' was this") —
the lesson was recorded but the second launcher was never fixed.

**Controlled proof — frozen SurgeNetXL, weights cannot change, 3 seeds per arm:**

| protocol | IVT mAP | mean | val meanF1 | clipper AP | "clip" verb AP |
|---|---|---:|---:|---:|---:|
| TPN=4  | 25.86 / 26.24 / 26.00 | **26.03** | ~78.0 | 66.71% | 66.61% |
| TPN=12 | 21.95 / 22.48 / 21.74 | **22.06** | ~75.5 | 40.80% | 39.07% |

**−3.97 IVT from protocol alone**, seed spread ~0.3. The "clipper-class signature"
reproduces on an encoder that never trained — it is low-support-class sensitivity to
head undertraining, not a rare-class representation deficit.

**Per-class AP makes the mechanism explicit** (frozen SNX, mean of 3 seeds) — the
damage scales inversely with class support, and is essentially confined to the
rarest class:

| class | support (clips) | TPN=4 | TPN=12 | Δ |
|---|---:|---:|---:|---:|
| **clipper** | 115 (6.7%) | 67.9 | 44.0 | **−23.9** |
| grasper | 1526 | 98.3 | 96.3 | −2.0 |
| irrigator | 330 | 89.5 | 87.9 | −1.6 |
| scissors | 259 | 90.0 | 89.2 | −0.8 |
| hook | 1136 | 98.9 | 98.7 | −0.2 |

Cutting optimizer steps 222→74 at unchanged LR starves the classes with the fewest
gradient events per epoch; common classes barely move. IVT then amplifies this into
a ~4-point headline drop because it multiplies per-task probabilities across 360
triplet combinations, so a single degraded axis propagates into every triplet
containing it.

**Arm inventory (verified from job logs, not inferred from directory names):**
- **TPN=4 / 222 iters — clean:** all `abl_*` composition ablations (openhin,
  croppedopenh, lemonout, gynout, lapout, robonly, full), `noopenh_e79`,
  `sitlonly_e79`, and every §1 headline arm (meta1b/2b, ours1b_e19, ours2b_e159,
  samp_t050/t075).
- **TPN=12 / 74 iters — contaminated:** all 15 `v2_*` triplet arms (e19→e324→final,
  both `_meanpool`, `surgonly_fresh`/`cooldown`, and the `_s1`/`_s2` replicates),
  prod9M e14/e19/e24, all prod18M arms, `snx_*_tpn12`.

**Directory suffix is NOT a reliable tell:** `v2_e324_s1` carries an `_s` suffix but
ran at TPN=12 — `triplet_diag_chain.sh` derives its seed from a trailing `_sN` in the
tag, so suffixed names appear in both families. Classify by logged `train_loss`
(~1.1 = TPN=4, ~0.11 = TPN=12) or by the job log's `world_size=`/`iterations per
epoch`, never by the directory name.

Because all `v2_*` arms share one protocol, their **internal** lineage ranking may
survive (a common offset largely cancels), but rare-class numbers are unreliable even
within that family, and any v2-vs-non-v2 triplet comparison is invalid. The GraSP
`v2_e324` results are unaffected (different code path).

This retroactively explains every open puzzle in the entry below: two seeds agreed
because seeds don't change the launcher; GraSP showed no dip because the cached
export+probe path never used the 12-rank launcher; effective rank / cosine similarity
/ per-channel variance were all normal because the encoder genuinely was fine.

**★ Mid-run confirmation across three disputed checkpoints (jobs 8762936, 8763075,
2026-08-18).** At matched epoch 6, re-probing at TPN=4 lifts every previously
"collapsed" checkpoint into the healthy band:

| checkpoint | verbF1 TPN12→TPN4 | targetF1 TPN12→TPN4 |
|---|---|---|
| prod9M_e14 | 71.67 → **77.90** | 56.45 → **63.52** |
| v2_e199 | 73.18 → **78.27** | 56.02 → **62.02** |
| prod18M_e59 | 72.45 → **77.62** | 56.86 → **66.10** |

All six TPN=4 arms then occupy ONE tight band at ep6 (verb 77.6–79.1, target
62.0–66.1) — including the e4/e9/e29 "healthy" anchors. The dip/no-dip distinction
vanishes once protocol is matched. (Mid-run F1 indicator; final IVT pending.)

**This extends the retraction to the v2 lineage's "late-CPT decay" on TRIPLET.** Under
TPN=12 the v2 triplet arms decline monotonically — e19 23.88, e79 22.70, e139 20.45,
e199 19.11, e259 19.26, e324 19.58, final 20.31 — a shape read elsewhere as real
late-CPT decay. v2_e199 is the trough and recovers to the healthy band at TPN=4, so
that decline is at least largely protocol, not training. **Re-probe the rest of the v2
lineage at TPN=4 before citing any of its triplet numbers.** Note this does NOT touch
the GraSP `v2_final < v2_e324` observation above — GraSP used a different code path
and is unaffected.

**★★★ IVT CONFIRMATION — prod_18M e59 fully recovers (job 8762936, 3 seeds, scored at
ep17/25; the probe writes prob dumps every epoch).**

| protocol | IVT (3 seeds) | mean | clipper AP | "clip" verb AP |
|---|---|---:|---:|---:|
| TPN=12 | 21.27 / 19.75 | ~20.5 | 61.23% | 31.62% |
| **TPN=4** | **30.43 / 29.04 / 30.25** | **~29.9** | **81.98%** | **81.61%** |

**+9.4 IVT.** e59 was THE checkpoint whose "stable collapse, reproducible across 2
seeds" was the headline evidence that doubling the pretraining budget failed on
Triplet. At matched protocol it lands at the **top of the all-encoder band** (raw
meta2b 29.97, CPT arms 29.5–29.6) — an entirely normal checkpoint. The rare-class
signature is gone: clipper and "clip" both sit at/above the ~78–80% healthy baseline.

**Therefore the claim "the 2x-budget prod_18M run produced a checkpoint that is
flat-to-tied on GraSP and measurably worse on Triplet" is FALSE and is retracted.**
The Triplet half was the launcher. (The GraSP half — a genuine tie at 71.05/72.82 vs
70.98/73.52 — used a different code path and still stands.)

**On magnitude (updated).** The frozen-SNX control put the protocol effect at −3.97
IVT, which suggested a residual the confound could not explain. The direct e59
re-probe supersedes that estimate for V-JEPA checkpoints: the observed recovery is
**+9.4 IVT**, i.e. protocol accounts for the *entire* gap and the SNX figure
understated it. That is consistent with SNX being a different architecture that
absorbs head-undertraining differently — the reason the earlier caveat flagged it as
an imperfect proxy. **No residual effect remains to be explained for e59.**

The e59 read is converged, not a lucky epoch: re-scoring at ep19 gives 30.30 / 29.12 /
30.09 vs ep17's 30.43 / 29.04 / 30.25 — every seed moves ≤0.16. Seed spread (~0.6)
is back in the 0.24–0.90 range normal for this probe, versus the ~10-point swings the
confound produced.

**★★★ ALL THREE DISPUTED CHECKPOINTS NOW CONFIRMED ON IVT** (chain 8763075→, scored
at ep12/25 — still climbing, e59 gained ~1.5 pts between ep12 and ep19, so these are
lower bounds):

| checkpoint | TPN=12 (void) | TPN=4, 3 seeds | mean | Δ |
|---|---|---|---:|---:|
| prod18M_e59 | 21.27 / 19.75 | 30.34 / 29.23 / 30.45 (**final ep25**) | **30.01 ± 0.67** | **+9.50** |
| prod9M_e14 | 18.02 / 21.25 | 30.48 / 29.70 / 29.73 (ep24) | **29.97 ± 0.44** | **+10.34** |
| v2_e199 | 19.11 | 29.16 / 27.43 / 29.13 (ep24) | **28.57 ± 0.99** | **+9.46** |

(All three converged: ep18→ep24 drift ≤0.10 on the two chain arms, ep17→ep25 ≤0.16
on e59. Seed spreads 0.44–0.99 are in this probe's normal 0.24–0.90 range, versus the
~10-point swings the confound produced.)

Clipper AP — the class that defined the "rare-class discrimination deficit":

| checkpoint | TPN=12 | TPN=4 |
|---|---:|---:|
| prod9M_e14 | 32.38% | **82.95%** |
| v2_e199 | 34.69% | **72.40%** |
| prod18M_e59 | 61.23% | **81.98%** |

prod9M_e14's clipper AP — the single number the whole rare-class theory rested on —
more than doubles and lands *above* the ~78–80% healthy baseline. **There was never a
rare-class representation deficit, a mid-run dip, or a late-CPT decay on Triplet.
Every one of them was the probe launcher.**

**★★★ COMPLETE CORRECTED TRAJECTORIES (closeout chain 8763331→, 1 seed each, ep24).**
Every arm re-probed at TPN=4 clears the collapsed band. The dip is gone from prod9M
entirely, and the v2 "late-CPT decay" shrinks to a mild, plausibly-real decline:

| prod9M | TPN=12 (void) | TPN=4 |
|---|---:|---:|
| e4 | 29.04 | *(already TPN=4)* |
| e9 | 28.85 | *(already TPN=4)* |
| e14 | 19.63 | **29.97 ± 0.44** (3 seeds) |
| e19 | 19.84 | **29.14** |
| e24 | 20.03 | **27.62** |
| e29 | 28.45 | *(already TPN=4)* |

**No dip remains** — the trajectory is flat-to-gently-declining across 1.54M→9.22M
samples, entirely inside the encoder band. The "sustained ~500-step-wide dip that
fully recovers" was three consecutive checkpoints that happened to be probed by the
other launcher.

| v2 | TPN=12 (void) | TPN=4 | Δ |
|---|---:|---:|---:|
| e19 | 23.88 | **29.41** | +5.53 |
| e139 | 20.45 | **28.95** | +8.50 |
| e199 | 19.11 | **28.57 ± 0.99** (3 seeds) | +9.46 |
| e324 | 19.58 | **26.54** | +6.96 |
| final | 20.31 | **26.45** | +6.14 |

**The v2 late-CPT decay is REAL but much smaller than reported**: 29.41→26.45 (−2.96)
rather than 23.88→20.31 in a collapsed band. Note the correction is *not* a constant
offset (+5.5 to +9.5), so protocol interacted with checkpoint — one more reason the
TPN=12 numbers cannot be rescued by subtracting a constant. Single seed each; the
−2.96 spans roughly 3× the 3-seed spreads measured here (0.44–0.99), so it is
probably real, but seed-replicate e19 and final before citing a magnitude.

**Caveat on all closeout numbers:** 1 seed, scored at ep24. `probe_done` accepted
`last >= target-1`, so the chain self-terminated at ep24 rather than ep25 — **this
was NOT by design, it was an off-by-one bug**, fixed 2026-08-18/19 to `last >= target`
(see [[triplet-chain-offbyone-and-scorer-cache]] in memory). Re-training the true
epoch 25 for the two affected multi-seed arms and force-rescoring moved both by only
+0.03 IVT (`prod9M_e14tpn4` 29.97→30.00, `v2_e199tpn4` 28.57→28.60) — noise-level, so
none of the closeout numbers above need revision. The bug separately caused the
`triplet_lrsweep_final` LR-sweep job to skip its last epoch entirely (see the LR
sweep entry below), where the effect was NOT negligible.

**Tooling bug worth remembering:** `sed 's#v2_final#v2_finaltpn4#g'` on the config
also rewrote the *parent directory* `surg_2_1_v2_final` → `surg_2_1_v2_finaltpn4`,
so that arm ran to a stray tree (`/flare/ModCon/ngetty/surg_2_1_v2_finaltpn4/...`).
The run is valid and was scored from there, but the tag-rename sed used throughout
this work is unsafe whenever a tag is a substring of its own output path. Until those land, treat
prod9M e19/e24 and the rest of the v2 lineage as unmeasured rather than collapsed.

Unresolved side-anomaly, same split: logged `train_loss` is ~1.1 (TPN=4) vs ~0.11
(TPN=12), with epoch 1 of a TPN=12 run logging at the normal ~2.0 scale before
dropping 10× at epoch 2, and again after each resume. `AllReduce` in
`src/utils/distributed.py` *does* divide by world size, so it is not the cause. This
does not affect the confound conclusion but is worth its own look.

Hygiene note: `evals/triplet_recog_frozen/` is **entirely untracked in git** (`??`),
so probe-code changes leave no history — a standing hazard for confounds like this.

---

### `vitG384_prod_18M` — GraSP + Triplet trajectory, and the Triplet mid-run dip (2026-08-17) — ⚠️ SUPERSEDED BY THE RETRACTION ABOVE

This is the 2x-budget follow-up run referenced above (256n, `configs/vitg16_surg_vid_webdataset_single4/vitG384_prod_18M.yaml`,
job 8760386, writing to `/flare/ModCon/ngetty/checkpoints/daos_shakeout/vitG384_prod_18M_n256_8760386/`,
**completed 2026-08-17, e59 final at 18.43M samples**). Probed with the same GraSP (cached,
ctx3 builder, 3-LR-head sweep) + Triplet (live, 25-epoch, topk_mean k=8) protocol as the
prod_9M trajectory above, cloned via `scripts/gen_prod18m_grasp_triplet_configs.py`.

**Initial read (now corrected below) at e19 (6.14M samples — see note on the sample-count
error):** Triplet IVT mAP = 19.06, far below the prod_9M trajectory's ~28.6 at the naively
"matched" sample count. The first hypothesis — that this was purely prod_18M's 60-epoch
schedule being less-annealed than prod_9M's 30-epoch schedule at the same step — **was
disproven** by the investigation below. Two things were wrong with the initial read: (1) the
"~9.5M samples" figure for e19 was a hand-computed error — `ipe=50` fixed in both configs
means `eN` = step `(N+1)*50` regardless of total epochs, so e19 is actually **6.14M samples**
(20 completed epochs × 50 × gb 6144), not 9.5M; (2) prod_9M's OWN e19 (same corpus, same exact
step count, never before probed — the original trajectory only sampled e4/e9/e29) turned out
to show the **same collapse**, ruling out anything specific to prod_18M's schedule.

**Dense trajectory (both runs), Triplet IVT mAP — jobs 8761834/8761841→8761920/8761954 (prod_9M
e14/e19/e24 control probes) and 8761957→8762104 (prod_18M e59 final)::**

| ckpt | samples (ipe=50 fixed) | prod_9M IVT | prod_18M IVT |
|---|---|---:|---:|
| e4  | 1.54M | 29.04 | — |
| e9  | 3.07M | 28.85 | — |
| e14 | 4.61M | **18.02** | — |
| e19 | 6.14M | **19.84** | **19.06** |
| e24 | 7.68M | **20.03** | — |
| e29 (prod_9M final) | 9.22M | 28.45 | — |
| e59 (prod_18M final) | 18.43M | — | **21.27** |

**★★★ prod_9M shows a real, sustained, reproducible dip-and-recover in Triplet IVT — not a
single noisy checkpoint, not a data/sampling artifact, not an LR-schedule mismatch.** Ruled
out, each with direct evidence:
- **Not a sampling/corpus difference**: `params-pretrain.yaml` on disk for both runs shows
  byte-identical `datasets`/`datasets_weights`/`dataset_fpcs`/`sampling_temperature=0.5` (14
  sources, weights all 1). `src/datasets/webdataset.py`'s `compute_mixing_probs` depends only
  on `sample_counts`/weights/temperature, never on configured `epochs` — confirmed via both
  runs' startup logs showing byte-identical `realized=` per-source percentages
  (`ranklogs_8758240/rank.0.out` vs `ranklogs_8760386_attempt1/rank.0.out`). At matched step
  count both runs have drawn the exact same cumulative exposure per source.
- **Not an LR/lambda schedule artifact**: LR decays smoothly and monotonically through the
  entire dip window in prod_9M's own schedule (verified numerically: lr_frac 0.99→0.84→0.59→0.30→0.08→0.00
  from e4→e29) — IVT is **non-monotonic** against a monotonic LR (high at e4 when LR is
  highest, dips mid-decay, recovers as LR→0), which a "less annealed" story cannot explain.
  Lambda is already saturated at its terminal value 0.5 for the entire dip window in both
  configs (`lambda_end_iter` reached well before e14 in both schedules).
- **Not a JEPA pretraining loss anomaly**: per-epoch average loss is smooth through the dip in
  both runs (prod_9M ~0.41 flat across e9-e19, no spike, no discontinuity) — if the training
  dynamics were corrupting the representation, the self-supervised loss itself would show it.
- **Not global representation collapse**: measured effective rank (entropy of the singular
  value spectrum), pairwise cosine similarity, and per-channel variance directly on the
  exported GraSP feature caches (e4/e9/e14/e29, ~4000 pooled tokens each). None show a
  collapse signature tracking the dip — effective rank declines smoothly and monotonically
  (152→148→134→129, e4→e29, not U-shaped), cosine similarity is flat-to-rising (0.42→0.41→0.43→0.52),
  zero near-dead channels at any checkpoint. A DINO/BYOL-style collapse would show a sharp,
  reversible drop in rank/uniformity at the dip; none appears.
- **★★★ Not encoder-wide at all — cross-probe test with GraSP is negative.** GraSP e14
  (export+head-train jobs 8762019/8762091→8762216, early-stopped ep12 best@ep6, scored job
  8762397) = best-head 68.13 / ensemble 70.86, landing exactly on the smooth e9→e29 trend
  (68.42→70.98 / 71.66→73.52) — **no dip at all** on a completely different task (11-class
  phase recognition), different probe head, different label space, same checkpoint. If the
  frozen encoder's representation had actually degraded at this step, GraSP would show it too.

**What the dip actually is: a Triplet-probe-specific rare-class discrimination failure,
concentrated in the minority classes, and CONFIRMED REPRODUCIBLE ACROSS SEEDS — not
probe-training noise.** Per-class tool AP, prod_9M e9 (healthy) → e14 (dip) → e29 (recovered):
clipper (115/1704 clips, 6.7% support) 80.14%→**32.38%**→78.30%; irrigator (330 clips)
90.21%→65.38%→87.73%; hook (1136 clips) 98.56%→84.10%→92.02%; grasper (1526 clips) and
scissors (259 clips) stay essentially flat (97.63→96.34→97.80 and 91.60→89.17→91.38). The
matching verb axis shows the identical signature: "clip" (the clipper's action, same 115
clips) 80.05%→**25.86%**→80.87%, while coagulation/grasp/retract/clean stay flat. **Excluding
clipper entirely still leaves a real drop (4-class avg 94.5→83.7, e9→e14)**, so this is not
purely a single-class artifact — it's a broader but still support-correlated weakness (hook,
at 1136 clips, also drops 14.5pt).

**★★★ A first draft of this entry (now corrected) hypothesized "probe-training noise" — a
linear head landing in a bad local optimum for minority classes at this checkpoint, distinct
seed-to-seed. This was WRONG and is retracted.** A second independent seed was run at e14
(job 8762459→8762617, `trip_prod9M_e14_s1.yaml`, `VJEPA_PROBE_SEED=1`) specifically to test
this: **seed 1 = IVT 21.25 (clipper AP 29.53%, clip-verb AP 33.69%), seed 0 = IVT 18.02
(clipper 32.38%, clip-verb 25.86%)** — both seeds land within 3 points of each other, both
firmly in the collapsed regime, both showing the same clipper-specific signature. Every
triplet arm previously measured in this project has shown seed-to-seed std of 0.24–0.90 IVT
points ([[sar-seed-ensemble-result]] and the ★ tables above) — a reproducible ~10-point drop
across 2 independent seeds is categorically not that kind of noise. **This checkpoint has a
real, reproducible discrimination deficit for the clipper instrument (and its associated
verb) at this point in training, that recovers by e29.** The mechanism is still unresolved:
GraSP at the identical checkpoint (e14) shows no dip at all (68.13 best-head / 70.86 ensemble,
right on the smooth e9→e29 trend) — proving the frozen encoder's representation is not
globally degraded — so whatever is happening is specific to how this particular checkpoint
state represents/exposes clipper-class information to a linear/shallow readout, not a
uniform representation collapse. Effective rank, pairwise cosine similarity, per-channel
variance, and token-norm-outlier checks on GraSP's exported feature cache (a different eval
clip set than Triplet's own val set, same encoder forward pass) found nothing anomalous at
e14 — but this does not rule out a clipper-class-specific effect invisible to a global,
un-stratified feature-cache statistic. **Open**: has not yet inspected the embeddings of
Triplet's own held-out clipper-class clips directly.

**★★★ CONFIRMED: prod_18M's final checkpoint (e59) is STABLY collapsed on Triplet, and did
NOT recover the way prod_9M's e29 did — this is a real, actionable finding, not noise.** Two
seeds (jobs 8762681→8762814 seed1, plus the original seed0): **seed0 IVT=21.27, seed1
IVT=19.75** — within 1.5 points of each other, mean ~20.5, both firmly in the collapsed
regime (clipper AP 22.6%[seed0]/40.7%[seed1] vs the ~78-80% healthy baseline; "clip" verb
similarly depressed). Doubling the pretraining budget (9.22M→18.43M samples) **did not
recover Triplet performance the way prod_9M's own trajectory did by its natural endpoint.**

**GraSP tells the complementary story: essentially no gain from the 2x budget either.**
prod_18M's e59 GraSP (export+head-train jobs 8762450/8762493, early-stopped ep12 best@ep6,
scored job 8762717): **best-head 71.05, ensemble 72.82** — a statistical tie with prod_9M's
e29 (70.98 best-head, +0.07; 73.52 ensemble, actually −0.70), both differences well inside
the ~3 mAP single-seed noise floor. **Doubling the budget bought no clear GraSP improvement
either.**

**Net read: the 2x-budget prod_18M run did not produce a better final checkpoint than
prod_9M by either metric** — flat-to-tied on GraSP, and measurably, reproducibly worse on
Triplet (mean ~20.5 vs prod_9M's single-seed 28.45 at e29). This raises the open question of
whether prod_9M's own e29 Triplet recovery is itself a fragile, single-seed-not-yet-confirmed
result (only ever measured once) — **a 3-seed re-probe of prod_9M's own e29 is now the
natural next step**, to confirm the "recovery" is as real/stable as the "dip" has now been
shown to be, before concluding prod_18M's run specifically failed to reach it (as opposed to
prod_9M's e29 recovery itself being a lucky single seed). Whatever the mechanism, it is NOT a
dataset-weighting, sampling-temperature, or corpus difference — both runs verified
byte-identical on every sampling/corpus axis checked. The mechanism is unresolved: GraSP
proves the frozen encoder representation is not globally degraded at the collapsed
checkpoints, so this is specific to Triplet's rare-class (clipper/clip) discriminability, not
a uniform representation defect — but why prod_18M's longer schedule failed to exit this
regime by 2x the step count of prod_9M's own recovery point is not yet understood.
...
**★★ That localisation margin is a PRETRAINING-FAMILY property, not something our CPT bought.**
Every V-JEPA row — including *raw Meta 2B frozen*, the weakest detector in the table at AP_mean
0.1278 — has boxAP50† 0.39–0.52, while both externals sit at 0.21–0.22 whether frozen or
fine-tuned. The single best box localiser on record is **surgical 2B e159 frozen at 0.5156**,
above even prod37M FT+aug's 0.5101, despite ranking 11th of 16 on detection AP. boxAP50† does
not track detection AP across our checkpoints (Spearman ρ = **0.19** over the 8 V-JEPA rows with
oracle columns — weak and well inside the noise at that n). **Presence is what separates our
checkpoints from each other; box quality is what separates the V-JEPA family from the
image-encoder externals.** Two consequences: (a) do not rank our own checkpoints by boxAP50†;
(b) an external's poor detection AP is fairly attributed to localisation, but our own CPT gains
are not.
...
|---|---|---|---|---|---|---|
| base | 3 | 0.1470 | 0.1340 / 1 / 0.1560 | 0.0220 | 0.0131 | 0.2294 |
...
### `vitG384_prod_9M` final checkpoint (e29) — ESAD frozen, 3-seed + seed-ensemble (2026-08-17)

Fills the ESAD half of the same gap SAR closed above. Checkpoint slimmed
(`scripts/slim_ckpt_for_probe.py`, target_encoder-only, 7.4 GB) and staged to eagle
(`/eagle/projects/ModCon/ngetty/checkpoints/esad_probe_slim/prod9M_e29_target_encoder.pth.tar`,
matching the naming convention of the other 4 slimmed checkpoints already there), then run
through the existing Polaris pipeline (`export_esad_cache.py` + `train_esad_double_head.py` +
`score_esad_detection_ap.py`, same as every other arm) — no Aurora path exists for ESAD, all
prior arms went through this same eagle/Polaris route. Cloned `esad_double_export_polaris.yaml`
→ `esad_double_export_polaris_prod9m_e29.yaml` (only the `checkpoint:` path differs; embed_dim
1664/`vit_gigantic_xformers` unchanged since prod_9M is the same 2B architecture as e159).

**Caught and fixed a protocol bug during this run:** the repo's `run_esad_double_probe_polaris.sh`
never threads `--seed` through to `train_esad_double_head.py`, so a first attempt (job 7484330)
trained seed 0 **unseeded** — discarded (moved to `..._s0_UNSEEDED_discard`) and retrained
properly with a new `scripts/run_esad_prod9m_seed_train.sh` that explicitly passes `--seed`,
reusing the seed-0 export cache (frozen encoder → identical features across seeds, only
head-train changes). Each seed needed 1-2 walltime resubmits on the 1h `debug` queue (seed0:
jobs 7484330[discarded]/7484638/7484940; seed1: 7484953/7485160; seed2: 7485279/7485498), every
resume confirmed via the trainer's own `[TRAIN] resumed from epoch N` log line. Scored on the
same full-denominator (`gt_full=11207`, `gt_cov=10619`, `frames=5903`) protocol as every other
row in §4e (jobs 7484347 for the cov_p0/p1 test-window export, 7484952/7485274/7485576 for
per-seed scoring). Seed-ensemble scored via `score_esad_seed_ensemble.py` (job 7485869,
`capacity` queue, 4h walltime — a first `debug`-queue attempt (job 7485693) hit exactly the
documented ~76-80 min ensemble-scoring wall from §4f and was killed at 1h mid-computation with
no partial ensemble result; the per-seed baselines it inferred were preserved and reused on
resubmit, only the ensemble step reran).

| encoder | frozen (3-seed mean ± spread) | seed-ensemble |
|---|---:|---:|
| CPT 2B (prod9M_e29) | 0.1420 ± 0.0075 | 0.1508 |
| CPT 2B (e159), for reference | 0.1483 ± 0.0149 | 0.1569 |

Per-seed: 0.1355 / 0.1502 / 0.1403.

**Reading: prod9M_e29 sits inside the existing tie band (0.127–0.154) this section already
established — a tie with fs_e159, not a win, not a loss.** Unlike SAR (where prod9M_e29 clearly
separated from the saturated band, +1.13 over fs_e159, outside fs_e159's own spread), ESAD shows
no comparable movement — prod9M_e29's mean (0.1420) is well within fs_e159's ±0.0149 spread and
vice versa. This asymmetry (SAR moves, ESAD/GraSP-trajectory/triplet-trajectory don't, for the
same checkpoint) is itself the interesting open question, flagged in the summary's Open section.
Seed-ensembling grows prod9M_e29 modestly (0.1420→0.1508, +0.0088), smaller than fs_e159's own
ensemble gain (0.1483→0.1569, +0.0086) — comparable magnitude, no differential signal from the
ensembling axis either.

**Same protocol-currency caveat as every row in §4e/4f/4g:** this uses the pre-fix (`union`-mode)
presence labeling — the 2026-08-17 presence-label defect fix (§4g above) has not yet been applied
to ANY checkpoint's manifests (fix is committed, reruns pending), so this number is consistent
with, not stale relative to, every other row currently in this table.

Reproduce: `scripts/slim_ckpt_for_probe.py`, `scripts/run_esad_prod9m_seed_train.sh`,
`scripts/run_esad_prod9m_cov_export.sh`, `scripts/score_esad_prod9m_seed.sh` (all in this repo,
staged to eagle's `/eagle/projects/tpc/leonardo_borgioli/esad_probe/`); results at
`/eagle/projects/ModCon/ngetty/esad_probe/runs/esad_double_prod9m_e29_frozen_s{0,1,2}/test_detection_ap_cov_fulldenom.json`
and `/eagle/projects/ModCon/ngetty/esad_probe/seed_ensemble_scores/prod9m_e29_seed_ensemble.json`.
...
**Oracle columns for the FT externals (2026-08-19, job 175739/40/41/56).** Presence mAP /
mean IoU / [email protected] were requested for the FT-vs-FT table but only ever computed for e159
FT (§4b-unfreeze: 0.3158 / 0.4536 / 0.4497). Extending to the externals hit the SAME
tubelet-geometry bug class this whole investigation keeps finding: `run_oracle_external.sh`
was pointed at the V-JEPA two-phase tubelet=2 caches (`cache_ft_<arm>_s<seed>_cov_p{0,1}`,
temporal_tokens=24) instead of the externals' own single tubelet=1 cache
(`cache_ftext_<arm>_s<seed>`, temporal_tokens=48) — same mismatch `score_ft_external.sh`
already had to route around for the AP table above. It didn't mis-score silently; the ragged
presence tensor crashed `average_precision`'s indexing (`IndexError`), so this was caught at
run time, not by comparing against a sanity number. Fixed by pointing at the correct cache;
also missing a Sophia free-GPU guard that `score_ft_external.sh` already carries — one job
(lemonfm s2) died silently at 44s (`Exit_status=143`, no traceback) from GPU co-tenancy before
that guard was added here too. **⚠ Oracle metrics are gated on GT presence being supplied for
free — see the caveat at §4b-unfreeze before citing these against any challenge/paper number.**

| arm | presence mAP (well-supported) | mean IoU | box mAP@50 |
|---|---|---|---|
| e159 FT (n=3, §4b-unfreeze) | 0.3158 ± 0.0202 | 0.4536 ± 0.0132 | 0.4497 ± 0.0612 |
| **LemonFM FT** (n=3) | 0.3118 ± 0.0078 | 0.3488 ± 0.0056 | 0.2242 ± 0.0064 |
| **SurgeNetXL FT** (n=3) | 0.2711 ± 0.0254 | 0.3318 ± 0.0083 | 0.2082 ± 0.0180 |

Per-seed: LemonFM well=0.3098/0.3204/0.3051, iou=0.3424/0.3517/0.3523, map50=0.2191/0.2221/
0.2313; SurgeNetXL well=0.3003/0.2586/0.2543, iou=0.3281/0.3261/0.3413, map50=0.1912/0.2065/
0.2270. Completed 2026-08-20 (job 175756, s2 landed after a Sophia by-gpu-queue backlog
cleared). meta1b/meta2b/ours1b_e19 FT oracle metrics were never computed (only e159 has a
job-171475-style oracle pass); computing them is straightforward with the existing
`score_oracle.py` path but was out of scope for this pass, which targeted the
external-vs-external gap specifically.

**Reading the oracle columns, with the same caution as the AP table above.** e159 clearly leads
on mean IoU and box mAP@50 (0.4536/0.4497 vs LemonFM's 0.3488/0.2242 and SurgeNetXL's
0.3318/0.2082) — both externals sit well below on localization quality once given the class for
free. Presence mAP is closer (0.3158 vs 0.3118/0.2711); LemonFM's tight n=3 spread (±0.0078)
makes its presence-mAP gap to e159 (−0.0040) look small, but there is no noise-floor estimate
for the oracle metrics the way one exists for detection AP
([[esad-probe-unseeded-noise-floor]] was derived from AP_mean only), so do not assign these
columns the same confidence language as the AP_mean claims above — **this remains directional,
not certified**.
...
### 4j. `vitG384_prod_37M` mid-run checkpoint (e54) — ESAD frozen, 3-seed (2026-08-20)

The next production checkpoint after prod9M_e29/prod18M_e59 in the same lineage — 2x
prod_18M's budget by schedule (`scripts/gen_prod37m_probe_configs.py`), but this probe uses
**e54 of a 120-epoch run (job chain 8766188, currently still training)**, i.e. a mid-run
checkpoint at ~16.9M samples seen, not the lineage's final checkpoint. Same 2B
`vit_gigantic_xformers` architecture as e159/prod9M_e29/prod18M_e59 (embed_dim=1664), so the
comparison here is CPT-recipe/budget, not scale.

Checkpoint slimmed (`scripts/slim_ckpt_for_probe.py`, target_encoder-only, 7.4 GB) from
`/flare/ModCon/ngetty/checkpoints/daos_shakeout/vitG384_prod_37M_n256_8766188/e54.pth.tar` and
staged to eagle (`/eagle/projects/ModCon/ngetty/checkpoints/esad_probe_slim/prod37M_e54_target_encoder.pth.tar`,
size-verified byte-identical both sides), then run through the same Polaris pipeline as every
other arm (`export_esad_cache.py` + `train_esad_double_head.py` + `score_esad_detection_ap.py`).
New launcher scripts cloned from the prod9M_e29 pattern: `run_esad_prod37m_export_seed0.sh`
(export train/val/test + seed0 train), `run_esad_prod37m_seed_train.sh` (seed1/seed2, reusing
the seed0 export cache), `run_esad_prod37m_cov_export.sh` (padded test-window cov_p0/p1 caches
for full-denominator scoring), `score_esad_prod37m_seed.sh` — all staged to eagle's
`/eagle/projects/tpc/leonardo_borgioli/esad_probe/`. New export/probe YAMLs
(`esad_double_export_polaris_prod37m_e54.yaml`, `esad_double_probe_prod37m_e54.yaml`) differ
from prod18M_e59's templates only in the `checkpoint:` path.

**Each training run needed 1-2 walltime resubmits on the 1h `debug`/`debug-scaling` queues**
(seed0: jobs 7529634[killed ep16]/7529865; seed1: 7529931[killed ep16]/7530849; seed2:
7529932[killed ep17]/7531215), every resume confirmed via the trainer's own `[TRAIN] resumed
from epoch N` log line. Scored on the same full-denominator (`gt_full=11207`, `gt_cov=10619`,
`frames=5903`) protocol as every other row in this section (job 7529935 for the cov_p0/p1
test-window export, 7531253/7531316/7531378 for per-seed scoring).

**Oracle metrics (presence well-supported mAP / macro mAP / mean IoU / box mAP@50), per seed:**

| seed | well_map | macro_map | mean_iou | map50 |
|---|---|---|---|---|
| 0 | 0.3330 | 0.2520 | 0.4911 | 0.5333 |
| 1 | 0.3507 | 0.2668 | 0.4778 | 0.4889 |
| 2 | 0.3929 | 0.3052 | 0.5011 | 0.5705 |

**Detection AP (full denominator), 3-seed:**

| encoder | frozen (3-seed mean ± spread) |
|---|---:|
| CPT 2B (prod37M_e54, mid-run) | **0.1571 ± 0.0208** |
| CPT 2B (prod9M_e29), for reference | 0.1420 ± 0.0075 |
| CPT 2B (e159), for reference | 0.1483 ± 0.0149 |

Per-seed: 0.1367 / 0.1565 / 0.1782.

**Reading: prod37M_e54's mean (0.1571) sits at the top edge of the existing tie band
(0.127–0.154) established across every other checkpoint in this section, and is nominally the
highest of the three CPT 2B checkpoints tested — but its own seed spread (±0.0208, the widest
of the three) fully overlaps both prod9M_e29's and e159's ranges, so this is NOT a resolved
win.** Note the direction is not monotonic in training budget either way to lean on: prod9M_e29
(9.22M samples) ties fs_e159, and prod37M_e54 (16.9M samples, more budget) nominally moves up —
consistent with, but not proof of, budget mattering here, since the same-checkpoint comparison
across this section's 3 CPT-2B rows has never cleared its own noise floor. **Same caution as
prod9M_e29's entry: SAR moves with budget for this lineage, ESAD/GraSP-trajectory/
triplet-trajectory do not (or do so only within noise) — this checkpoint doesn't change that
asymmetry.**

**This is a mid-run checkpoint (e54/120), not the lineage's final result** — re-probe once
`vitG384_prod_37M` completes if a fresh production checkpoint becomes the reference point,
the same way prod9M_e29/prod18M_e59 were each lineage's final checkpoint at the time they were
probed.

**Same protocol-currency caveat as every row in §4e/4f/4g/4h/4i**: this uses the pre-fix
(`union`-mode) presence labeling, consistent with every other row in this table (the
§4h `rep`-mode fix has not been extended to this checkpoint).

Reproduce: `scripts/slim_ckpt_for_probe.py`, `scripts/run_esad_prod37m_export_seed0.sh`,
`scripts/run_esad_prod37m_seed_train.sh`, `scripts/run_esad_prod37m_cov_export.sh`,
`scripts/score_esad_prod37m_seed.sh` (all in this repo, staged to eagle's
`/eagle/projects/tpc/leonardo_borgioli/esad_probe/`); results at
`/eagle/projects/ModCon/ngetty/esad_probe/runs/esad_double_prod37m_e54_frozen_s{0,1,2}/test_detection_ap_cov_fulldenom.json`.

### 4k. `vitG384_prod_37M` e54 — GraSP, Triplet, SSv2 forgetting (2026-08-20/21)

First checkpoint probe of the in-flight `vitG384_prod_37M` run (256n, kinetics400+openh
restored per [[prod-ssv2-forgetting-is-severe]]-equivalent memory and
[[corpus-decisions-rest-on-one-void-number]]), checkpoint e54 (54/120 epochs, 16.9M/36.86M
samples so far).

**GraSP** (cached export+probe, `eval_grasp_map_cached.py`, TAPIS-phase paper mAP, 3-LR-head
sweep, early-stopped ep12 best@ep6):

| ckpt | best-head mAP | ensemble mAP | top-2 ensemble mAP | head-train ep (best.pt) |
|---|---:|---:|---:|---|
| prod37M_e54 | **70.79** | **73.52** | 72.13 | ep6 |
| prod9M_e29 (for reference) | 70.98 | 73.52 | 73.22 | ep8 |
| prod18M_e59 (for reference) | 71.05 | 72.82 | — | ep6 |

**⚠ These numbers are on the OLD, build-confounded `ctx3` clip builder
(`build_grasp_asformer_ctx3_30fps.py`), NOT the fixed `official_ctx16` builder** — same defect
as flagged in the audit at the top of this document ("EVERY GraSP NUMBER IN THIS SECTION IS
BUILD-CONFOUNDED") and in the prod9M trajectory entry above. All three rows clone
`grasp_phase_fs_e159_export/probe.yaml` (verified: `scripts/gen_prod9m_grasp_triplet_configs.py`,
`gen_prod18m_grasp_triplet_configs.py`, `gen_prod37m_probe_configs.py` all load that exact
template), so the prod9M/18M/37M three-way comparison above is internally consistent, but
**none of these three numbers are comparable to the fixed-builder results elsewhere in this
ledger** — raw meta2b scores 75.65 best-head / 79.77 ensemble on the fixed build
([[grasp-fixed-build-beats-sota]]), and CPT `v2_e324` scores 79.68/82.00 on the same fixed
build — both well above every number in the table above. The fixed build is the one that
should be cited as "our best GraSP result"; the table above is a same-old-harness internal
comparison only, not a measure of how good this checkpoint actually is on GraSP. **Re-probe
prod37M_e54 (and ideally re-derive the prod9M/18M reference points) on `official_ctx16`
before drawing any conclusion from this table** — flagged after the user asked whether the
85.34/79.77-class fixed-build numbers were being conflated with this old-build table.

**Correction**: an earlier pass on this checkpoint reported `val_macro_f1=66.86` (read
directly from the head-training log, epoch 6) as the GraSP result. That is NOT the paper mAP
metric used everywhere else in this ledger and is not comparable to the rows above — it went
uncorrected until the user asked whether the ledger had been updated and flagged the number
as looking low. The `eval_grasp_map_cached.py` mAP above is the correct metric, but (per the
build-confound warning immediately above) is itself scored on the wrong builder.

**Triplet IVT mAP** — first pass used the WRONG protocol (TPN=12, the confound documented in
[[triplet-collapse-is-tpn12-probe-artifact]]) via a newly-written launcher script that copied
an unpatched template; scored a bogus **22.20%**. Re-probed correctly via
`triplet_seed_sweep.sh` (TPN=4, single seed): **30.89%** — a +8.69 point correction, landing
in the healthy band with raw meta2b (29.97) and prod18M_e59 (30.01, TPN=4-corrected). See
[[feedback-sweep-fixes-across-repo]] for the process gap that let the fixed bug recur.

**SSv2 forgetting** (10-epoch frozen probe, `configs/eval_2_1/forgetting_ssv2/`, multihead LR
sweep, same protocol as the prior forgetting investigation): **best val_acc = 57.76% @ ep9**
(healthy head, lr=0.0003) — notably ABOVE prod9M_e29 (52.57%) and prod18M_e59 (49.35%) at the
same probe protocol, despite comparable-or-greater CPT budget (54 epochs = 16.9M samples).
Still below raw meta2b (60.20%) and the old fs_e159 lineage (59.43%), so not fully recovered,
but this is the first direct sign that restoring kinetics400 rehearsal is arresting the
forgetting documented for the no-rehearsal prod9M/18M lineage — the opposite trend from that
lineage, where MORE budget without rehearsal produced WORSE forgetting.

**This was a mid-run checkpoint (e54/120) at the time of probing** — same caveat as §4j.
The run has since COMPLETED (2026-08-21, final checkpoint `e199.pth.tar`/`latest.pth.tar`,
see [[prod37m-run-complete]] in memory). Probe the final checkpoint next, using
`configs/heads/grasp/official_ctx16/grasp_off16_prod37M_e199_*` (the FIXED builder,
already pre-staged, untracked) for GraSP -- not `full_cached_384/` -- so the lineage's
actual final result is comparable to the fixed-build SOTA-beating numbers rather than
repeating the old-builder mistake this entry documents.

Reproduce: `scripts/gen_prod37m_probe_configs.py` (GraSP/Triplet/SSv2 config generation),
`scripts/score_prod37m_grasp.sh` (GraSP mAP scoring), `scripts/triplet_seed_sweep.sh` with
`ITEMS="prod37M_e54:0"` (correct-protocol triplet), `scripts/prod37m_grasp_triplet_ssv2_probe.sh`
+ `scripts/prod37m_probe_resume.sh` (SSv2/GraSP export+probe, walltime-resumed).
...
- [ ] **Does `prod9M`'s GraSP rise (67.63→69.18→73.52 best-head→ensemble, e4/e9/e29) continue
      past 9.22M samples, or is it flattening within the ~3 mAP noise floor?** Per-segment
      deltas are each within/at the noise floor; only the full e4→e29 delta clears it (see the
      `vitG384_prod_9M` trajectory entry in §3). User decision 2026-08-16: launch a NEW
      256n production config at **18.44M samples (2x)**, schedule (warmup/lr/lambda) recomputed
      for the larger budget, NOT a bare resume of `vitG384_prod_9M` past its own end (its
      cosine schedule had already decayed to `final_lr: 1.0e-06` by e29 — resuming under the
      same schedule would train at a near-zero, ineffective LR). Probe the new run's e29-equiv
      checkpoint on the same GraSP+triplet protocol. Also worth a 3-seed re-score of the
      EXISTING e29 checkpoint alone (cheap, reuses the built export cache) to tell whether
      70.98/73.52 itself is noise-floor luck before crediting the new run's comparison point.
...
## 7. SSv2 forgetting probe — catastrophic forgetting of general (non-surgical) video

Frozen attentive-probe on Something-Something-v2 (174 classes), 10 epochs, 3-head LR
sweep, config family `configs/eval_2_1/forgetting_ssv2/`. Purpose: does surgical CPT
degrade the encoder's general-video representation, and does that argue for including
non-surgical rehearsal data (kinetics400, pe_video) in the CPT corpus. This axis is
**invisible to every surgical downstream probe** (triplet, SAR, GraSP, ESAD) — none of
them can detect forgetting of capabilities they don't test.

### ★★★ Production checkpoints show SEVERE forgetting; the earlier "small" verdict used a lineage that (unlike production) HAD rehearsal (2026-08-19, corrected)

| checkpoint | corpus | ema | steps | val_acc @ ep10 | Δ vs raw meta2b |
|---|---|---|---:|---:|---:|
| raw meta2b | — | — | — | 60.20% | — |
| CPT 2B fs_e159 (OLD lineage) | **15 src, INCLUDES kinetics400** | 0.99925 | 9990 | 59.43% | −0.77 |
| **`prod9M_e29`** | 14 src, **no kinetics400** | 0.988 | 1500 (9.22M samples) | **52.57%** | **−7.63** |
| **`prod18M_e59`** | 14 src, **no kinetics400** | 0.988 | 3000 (18.43M samples) | **49.35%** | **−10.85** |

**Correction (2026-08-19): an earlier draft of this entry stated the fs_e159 lineage
had "zero rehearsal," which is FALSE — verified directly against
`configs/vitg16_surg_vid_webdataset_single4/vitG384_fixedshape.yaml`, whose 15-source
`datasets:` list includes `kinetics400` explicitly. The production configs
(`vitG384_prod_9M.yaml`, `vitG384_prod_18M.yaml`) have 14 sources with kinetics400
absent. So fs_e159 DID have general-video rehearsal and prod9M/prod18M do NOT — this
is a real, confirmed corpus difference between the two lineages, not a null control.**

The standing justification for excluding kinetics400/pe_video from the CPT corpus was
a single number from the fs_e159 lineage: raw meta2b 60.27 vs CPT 59.43 = **−0.84**,
read as "the entire forgetting cost, and it's small." That −0.84 was measured on a
run that ALREADY HAD kinetics400 rehearsal baked in — i.e. it was never a rehearsal-
free control, and its small forgetting number is consistent with (not evidence
against) rehearsal being the load-bearing variable. **Measured directly on the
production checkpoints — which dropped kinetics400 — forgetting is ~10× larger**, and
it gets WORSE with more CPT budget: prod18M (more surgical steps, same missing
rehearsal) forgot more than prod9M. At the current recipe, more surgical CPT budget
without rehearsal directly trades away general-video capability.

Scored via each checkpoint's own `log_r0.csv` `val_acc` at epoch 10 (multi-head sweep;
the reported head is whichever LR head wins — see the run-log caveat below). Jobs
8764801→8764925 (self-chaining, debug-scaling), both arms genuinely reached epoch
10/10 after a resubmit-race briefly killed the chain mid-slice (a second, unrelated
`qsub` landed in the same per-user queue slot as the chain's own self-resubmit,
which silently drops the successor with no error surfaced except in that job's own
`.OU` log — see `reference_aurora_debug_queues.md` in memory).

**Run-log caveat, same shape as the triplet TPN confound (§1) — read carefully before
citing per-iteration lines.** This probe trains 3 heads at once (`multihead_kwargs`:
lr=0.005, 0.001, 0.0003) in one DDP group, and the raw log prints one `loss:` line
per rank per head — the lr=0.005 head is *supposed* to diverge to `inf`/`1e17+` (it's
the throwaway aggressive-LR arm of the sweep); the surviving head is picked by
`[head-sweep] best=hN` and is what `log_r0.csv`'s `val_acc`/`val_macro_f1` columns
report. A raw `grep loss:` across the log looks like every arm is exploding — it
isn't. This exact log shape caused a same-session false alarm (job 8763883 was killed
believing it had diverged; it was healthy) before being understood — see
[[ssv2-probe-diverges-on-prod-checkpoints]] in memory.

**Confound, not yet resolved: kinetics-presence and EMA speed move together across
these two lineages, not independently.** fs_e159 has BOTH kinetics400 rehearsal AND
slow EMA; prod9M/prod18M have NEITHER. The two-lineage comparison alone cannot
separate them — either variable, or both jointly, could explain the −0.77 vs
−7.6/−10.85 gap. Untested:
1. Does adding kinetics400 back to the 14-source production corpus reduce forgetting
   at the CURRENT fast EMA (0.988)? 136,246 kinetics clips and 1969 pe_video shards
   are already staged; this is the direct, cheapest test.
2. Does slowing EMA alone (no corpus change) reduce forgetting on the current
   14-source corpus? This isolates the EMA-speed contribution independent of
   rehearsal.

### Potential impact on surgical domain adaptation strategy

**Two contributing factors, confounded together in the only two lineages measured so
far — both point toward the same fix (restore rehearsal), for two different reasons:**

**(a) Data-replay effect — real and load-bearing.** fs_e159 had kinetics400 in its
corpus; prod9M/prod18M do not. This is a textbook rehearsal removal, and a ~7-11
point val_acc cost is a large, plausible effect size for dropping a general-video
source outright (not diluting it — REMOVING it) during a surgery-heavy CPT run. The
direct, well-supported action: **restore kinetics400 (and consider pe_video) to the
next production corpus.** Both are already staged, so this is cheap to test in
isolation on the CURRENT (fast-EMA) recipe before touching anything else.

**(b) EMA-speed effect — plausible contributor, NOT yet isolated.** Independent of
the corpus difference, prod9M/prod18M also use a much faster EMA (0.988, ~83-step
averaging window) than fs_e159 (0.99925, ~1333-step window) over a much shorter run.
A slow-tracking target encoder changes slowly relative to the online encoder, which
can act as an implicit anchor to the (general-video) Meta initialization; a fast EMA
lets the target drift with the online encoder toward whatever the online encoder is
currently seeing. If the corpus lacks general-video data, a fast EMA would let both
encoders drift toward surgery-only representations largely unchecked — compounding
the data-replay effect rather than being a separate cause. **This is speculative
until an A/B holds corpus fixed and varies only EMA speed** — no such test exists yet
for this probe.

**Why both should be tested, not just (a).** [[next-256n-run-decision-inputs]]
already flagged that late-CPT decay on TRIPLET (a SURGICAL task) tracks EMA/run-length
across a *different* pair of lineages (fixedshape/v2 = slow EMA, decline; prod9M/
prod18M = fast EMA, no decline). If EMA speed genuinely modulates how fast the
encoder specializes toward its training distribution, that would explain BOTH
observations with one mechanism: fast EMA helps a surgical probe (faster
specialization into the training domain) and hurts a general-video probe (faster
drift away from the general-video init) by the same underlying process. Restoring
kinetics/pe_video alone (fix (a)) may not fully address this if EMA speed is doing
independent work — a corpus fix under a fast EMA could still under-perform a corpus
fix under a slower EMA. **Recommendation: before locking the next 256n corpus, run a
small matched 1B/short-budget screen that holds one variable fixed while varying the
other — e.g. add kinetics to the current 14-src+fast-EMA recipe first (cheapest,
addresses the now-confirmed rehearsal gap directly), and separately test a slower EMA
on the current 14-src corpus (isolates the EMA question) — reading BOTH SSv2 val_acc
and a surgical probe (triplet or SAR) in each case so the corpus fix and the EMA
question are evaluated on the same evidence rather than assumed to be one lever.**

Related: [[prod-ssv2-forgetting-is-severe]], [[next-256n-run-decision-inputs]],
[[budget-scaling-shows-up-in-finetune]], [[step-count-constants-break-at-scale]]
(EMA is a raw-step constant that does not renormalize with batch or run length)

## 8. Frozen-vs-FT gap on prod37M_e199 (Step 0 of the head-capacity/readout-depth plan, 2026-08-22)

**Baseline for `make-a-plan-for-rosy-pony.md`'s Exp-1/Exp-2.** The FT arms
(`tripft_prod37M_e199_s{0,1,2}`) had completed and dumped `val_probs_dump` NPZs but
were never scored — `map_summary.json` did not exist until this pass. Scored via
`scripts/compute_triplet_map.py` + `scripts/aggregate_triplet_seeds.py --ensemble`,
same canonical AP code as every other row in this ledger, so directly comparable.

| arm | tool | verb | target | mean-of-tasks | IVT mAP (mean±std) | IVT ensemble |
|---|---|---|---|---|---|---|
| frozen (`prod37M_e199`) | 89.72 | 78.92 | 54.13 | 74.25 | 31.18±1.64 | **32.36** |
| FT last-4 (`tripft_prod37M_e199`) | 93.17 | 84.33 | 59.54 | 79.01 | 37.68±0.23 | **38.69** |
| **gap (FT − frozen)** | +3.45 | +5.41 | +5.41 | +4.76 | +6.50 | **+6.33** |

Consistent with the prior +4.4–6.9 IVT range this plan was scoped around
(`docs/PROBE_RESULTS_LEDGER.md:302-333`), and the tightest seed spread seen on
triplet FT yet (std 0.23 vs frozen's 1.64) — FT is not just higher, it's more
stable, which matters for judging Exp-1/Exp-2 arms against a noisy target.
**This +6.33 IVT ensemble gap is the number Exp-1/Exp-2 arms are trying to close.**
Per the plan's success criterion, closing points alone is not sufficient — an arm
must also restore prod9M-vs-prod18M rank order (currently tied under frozen,
separated under FT at t=3.00, [[budget-scaling-shows-up-in-finetune]]).

Related: [[budget-scaling-shows-up-in-finetune]], [[ft-encoder-resume-bug]],
[[fine-tune-beats-frozen-readout]].

## 9. Exp-1 head-capacity and Exp-2 readout-depth results (`make-a-plan-for-rosy-pony.md`, 2026-08-22/23)

**IMPORTANT — Exp-1 is a head-recipe arm, not an encoder-comparison table.** Every
Exp-1 row is `prod37M_e199` with a *different head architecture*; nothing here should
be read into the encoder-comparison tables elsewhere in this ledger, which hold head
architecture fixed per `docs/PROBING_GUIDE_FOR_LEO.md:97-114`.

Both experiments needed one code change before they could run at all — logged for
anyone reusing this infra: (1) the triplet val-probs-dump only ever wrote classifier
index 0's predictions (`c_idx == 0` gate), so the plan's "3-LR sweep is free because
one encoder forward feeds 3 heads" claim would have silently discarded 2 of every 3
LRs' scores. Fixed by keying the dump `(c_idx, task_idx)` and writing non-zero
classifiers to a `head_<c_idx>/` subdirectory (`evals/triplet_recog_frozen/eval.py`);
`scripts/aggregate_triplet_seeds.py --head-subdir head_N` scores them. (2) The GraSP
export fanout script (`scripts/grasp_readout_export_fanout.sh`, written for this plan
to pack two 4-node exports into one `debug-scaling` allocation) initially omitted an
explicit `WORLD_SIZE` — two independent `mpiexec --pmi=pmix` sub-launches sharing one
PBS allocation's PMIx namespace is exactly the failure mode in
[[aurora-multi-mpi-per-pbs-worldsize]]: some ranks silently fell back to
`world_size=12` instead of the true 48, crashed `DistributedSampler`, and wrote a
completely empty, manifest-less cache while `mpiexec` still exited `rc=0`. Fixed by
setting `WORLD_SIZE` explicitly per sub-launch (matching `triplet_seed_sweep.sh`'s
existing pattern) and by having the wrapper verify actual manifest counts post-hoc
rather than trust the exit code.

### Exp-1: head capacity (triplet, `prod37M_e199`, 3 seeds, LR-swept)

All arms replace `token_aggregation`/`topk_mean` with `PooledFeatureMultiTaskClassifier`
(`head_type: pooled_feature`, `feature_pool: attention`) — never previously run in this
repo. Each arm swept 3 LRs (1e-3 / 3e-4 / 1e-4) sharing one encoder forward; **best LR
differs sharply by arm**, confirming the plan's premise that the one prior datapoint
(`attn_d2` = 40.3 tool mAP, `docs/TRIPLET_RESULTS_2026-07-01.md:94`) was LR-confounded:

| arm | lr=1e-3 | lr=3e-4 | lr=1e-4 | best mean±std | best ensemble |
|---|---|---|---|---|---|
| A0 baseline (`token_aggregation`, reused) | — | — | — | 31.18±1.64 | 32.36 |
| A1 attn_shallow (`num_probe_blocks=1`) | **34.66±0.62** | 33.86±0.75 | 33.48±0.37 | **34.66±0.62** | **37.06** |
| A2 attn_deep (`num_probe_blocks=4`) | 29.27±0.55 | 32.28±1.20 | **33.09±0.16** | 33.09±0.16 | 35.06 |
| A3 attn_mlp (blocks=1, `classifier_hidden_dims=[2048]`) | 32.59±1.44 | 32.89±0.56 | **33.58±0.68** | 33.58±0.68 | 35.78 |

A2's LR sensitivity is the clearest evidence the sweep was necessary: at lr=1e-3 it is
the *worst* arm (29.27, below baseline), at lr=1e-4 the *second-best* (33.09) — a
3.82-point swing from LR alone, on the same architecture. Had this plan run a single
default LR (the option explicitly rejected during planning), A2 would have looked like
capacity hurts, not "capacity needs a lower LR."

**Reading:** every attentive-pooling arm beats A0 at its best LR — A1 closes **~55%**
of the Step-0 FT−frozen ensemble gap (37.06 vs baseline 32.36, gap was 32.36→38.69)
with zero fine-tuning, zero new parameters beyond the ~microscopic attention pooler.
But per the plan's stated confound: A1–A3 change pooling (topk_mean → attention) AND
capacity simultaneously vs A0, and pooling alone is worth ~13 IVT on this task
(mean-pool 15.97 vs topk_mean 29.10 from prior work) — **this result cannot
distinguish "attentive pooling helps" from "capacity helps."** Depth alone (A1 vs A2:
34.66 vs 33.09) and width alone (A2 vs A3: 33.09 vs 33.58) show NO consistent gain
from adding capacity within the attentive-pooling family — if anything A1 (shallowest,
cheapest) wins. **Verdict: pooling change, not depth/width, is doing the work here.**
A clean capacity-only test (token_aggregation + added transformer blocks) does not
exist in the code and was flagged in the plan as the next step if results were
ambiguous — they are.

Cost: A2 (3 self-attention blocks over N=4608 tokens, D=1664) ran at **13.86 min/epoch
(1-seed smoke)**, ~1.6x the ~3h42m/25ep baseline reference (job 8773426) once scaled to
25 epochs (~5.8h). All 9 seed×arm runs (3 arms × 3 seeds) completed cleanly inside a
single 6h/15-node `capacity` job (rc=0 for all 15 items, including the 6 Exp-2-triplet
items packed into the same job — see below).

### Exp-2: readout depth (triplet + GraSP, block 47 baseline vs mid-layer readout)

New `readout_layer` flag (default `None`, verified bit-identical via a CPU-only unit
test: same output at `readout_layer=None` and at `readout_layer=<last hierarchical
layer>` as full-depth forward; early-exit at a genuine mid-layer produces a
differently-valued, correctly-shaped output) added to
`app/vjepa_2_1/models/vision_transformer.py` and both
`evals/{video_classification_frozen,triplet_recog_frozen}/modelcustom/vit_encoder_multiclip_v21.py`.
Confirmed on real hardware: exported/probed logs show `MID-LAYER READOUT ON: early-exit
after block 37 (of 48), reusing norms_block[2]` (and the analogous line for block 23)
for every rank, on both triplet (live) and GraSP (cached) paths.

**Triplet** (3 seeds, single default LR, no sweep — same config shape as A0):

| arm | mean±std | ensemble | Δ vs block-47 baseline (ensemble) |
|---|---|---|---|
| Block 47 (baseline, `token_aggregation`) | 31.18±1.64 | 32.36 | — |
| Block 37 | 31.86±1.30 | 33.65 | +1.29 |
| Block 23 | 21.35±1.21 | 22.92 | **−9.44** |

Block 37 is within-noise of the baseline (seed spread on this task is ~0.25–1.64 IVT
per the plan's own note) — no clear signal either way. Block 23 is a large,
seed-consistent regression (std 1.21, not overlapping baseline's range) — well past
the noise floor for triplet, unlike the ~3 mAP GraSP noise floor the −3.13 hierarchical
result was judged against ([[grasp-probe-noise-floor-3map]]). **Reading: for triplet,
de-specializing away from the final block does NOT recover the FT gap — it costs
nothing at block 37 and costs a lot at block 23.** This argues against "the last blocks
are over-specialized for pretext" as the dominant mechanism, at least for this task;
if it were, block 37 should have shown a positive effect commensurate with (or larger
than) block 23's regression, not a flat result at 37 and a cliff at 23.

**GraSP** (cached, ASFormer head, 3-LR sweep, `scripts/eval_grasp_map_cached.py`).
Export completed cleanly for both arms (48/48 rank manifests each, `readout_layer`
correctly recorded and matching in every manifest, verified directly against the
manifest JSON — not the export wrapper's own manifest-count check, which had an
off-by-factor-of-12 bug (`CHUNK_NODES` vs true mpiexec `world`) that falsely flagged
both healthy exports as incomplete; fixed in the script). Head-training needed 3
resubmits across `debug-scaling`/`capacity` (walltime kills at epoch 1, epoch 6/7, and
mid-epoch-20/18) — each resumed cleanly from `latest.pt`, confirmed by the log picking
up at the correct next epoch every time. Block 47's baseline head/cache did not exist
scored anywhere in this ledger for this exact checkpoint, so it was scored fresh here
for a same-day, same-cache-format, apples-to-apples comparison (all three arms score
via the identical `eval_grasp_map_cached.py` invocation against the GraSP phase TEST
split):

| arm | best head | ensemble (mean-of-3) | top-2 ensemble | Δ ensemble vs block-47 |
|---|---|---|---|---|
| Block 47 (baseline) | 82.64 | **84.84** | 84.38 | — |
| Block 37 | 76.56 | 80.97 | 79.79 | **−3.87** |
| Block 23 | 73.02 | 73.46 | 74.41 | **−11.38** |

**Reading: GraSP mAP falls monotonically with readout depth — the OPPOSITE of what
"the last blocks are over-specialized for pretext" would predict.** If de-specializing
away from block 47 recovered generalizable features, block 37 should have matched or
beaten the baseline; instead every arm loses points, and losses compound with distance
from the final block (−3.87 at block 37, −11.38 at block 23 — roughly 3x worse). This
agrees in *direction* with the triplet block-23 result (also a large regression) but
disagrees sharply on block 37: triplet showed block 37 as flat-to-slightly-positive
(+1.29 ensemble) while GraSP shows it as a clear, non-trivial loss (−3.87, well outside
either task's noise band — triplet's is ~0.25–1.64 IVT, GraSP's is ~3 mAP per
[[grasp-probe-noise-floor-3map]], and −3.87 sits right at that floor's edge while −11.38
is unambiguous). **Combined verdict across both tasks: readout depth is NOT a lever
that recovers the FT gap on either task, and for GraSP specifically the final block is
doing real, not just pretext-noise, work.** This directly refutes the plan's Exp-2
hypothesis as stated — the mechanism behind the FT gap is not "the final blocks are
too pretext-specialized to read out task-relevant features frozen"; if anything, the
final block is the MOST task-relevant one for GraSP among those tested. All three GraSP
arms remain competitive with or above the TAPIS paper SOTA reference printed by the
scorer (76.72 mAP v3, 76.07 v2) — even the worst arm (block 23, best-head 73.02) is
close to SOTA, and block 47's 84.84 ensemble clears it by +8.12 — so this is a
statement about the readout-depth LEVER, not about whether prod37M_e199 is a
competitive GraSP encoder (it clearly is, at block 47).

### Overall verdict against the plan's success criterion

Per the plan's stated bar: an arm counts as fixing frozen probing only if it *both*
closes a meaningful fraction of the gap *and* restores prod9M-vs-prod18M rank order
(currently tied under frozen, separated at t=3.00 under FT,
[[budget-scaling-shows-up-in-finetune]]). **Rank-order restoration was not tested in
this pass** — every Exp-1/Exp-2 arm above ran on `prod37M_e199` only, the same
checkpoint Step 0 anchored to. Closing the gap on one checkpoint's absolute score is
necessary but not sufficient by the plan's own bar.

With both experiments' data now in hand, the two mechanisms tested do NOT resolve
symmetrically:

- **Exp-2 (readout depth) is REFUTED on both tasks it was tested on.** GraSP mAP falls
  monotonically and substantially with readout depth (−3.87 at block 37, −11.38 at
  block 23, both outside noise); triplet shows the same regression at block 23 and no
  gain at block 37. There is no candidate arm from this experiment worth carrying
  forward to a prod9M/prod18M rank-order check — the readout-depth lever moves the
  wrong direction on the task (GraSP) where it moves at all with a clear signal.
- **Exp-1 (head capacity / pooling) is the only surviving candidate**, specifically A1
  (attentive-shallow, ensemble IVT 37.06, ~55% of the FT gap) — but per its own
  write-up above, the result is confounded with a pooling-mechanism change rather than
  isolating capacity, and was only tested on triplet, not GraSP. The next step is
  twofold: (a) run A1's `pooled_feature`/attention head on GraSP to check whether the
  ~55%-gap-closing effect generalizes across tasks the way Exp-2's regression did, and
  (b) a prod9M/prod18M rerun of A1 to check rank-order restoration, since closing
  absolute points on one checkpoint does not by itself satisfy the plan's bar.

Related: [[budget-scaling-shows-up-in-finetune]], [[triplet-collapse-is-tpn12-probe-artifact]],
[[grasp-probe-noise-floor-3map]], [[aurora-multi-mpi-per-pbs-worldsize]].

**Superseded by the two follow-ups below (2026-08-23/24):** (a) A1 stacked with FT
turned out to be a small regression vs FT alone, not additive — see "does A1's pooling
win stack with FT?" immediately below; (b) unfreezing MORE than last-4 (last-8) beat
FT-last4 outright (ensemble IVT 41.24 vs 38.69, +2.55) — see "FT unfreeze-depth sweep"
further below, which is now the best verified triplet result in this whole
investigation, surpassing everything discussed in this subsection. Rank-order
restoration (prod9M vs prod18M) remains untested for ANY arm, last-8 included — still
the open item before any of these can be called a complete fix per the plan's bar.

### Follow-up: does A1's pooling win stack with FT? (triplet, prod37M_e199, 2026-08-23)

Direct test of whether Exp-1's frozen pooling win and FT are additive: combined
`encoder_unfreeze_last_n: 4` with A1's winning head (`head_type: pooled_feature`,
`feature_pool: attention`, `num_probe_blocks: 1`, lr=1e-3 — both the FT baseline's
existing head LR and A1's winning LR, so no new LR choice was made).
`encoder_unfreeze_last_n>0` requires exactly one head (loss sums into one backward
against a shared trainable encoder), so no LR sweep here. 3 seeds, config
`configs/heads/triplet/finetune/tripft_pooled_prod37M_e199.yaml`, launcher
`scripts/tripft_pooled_seed_sweep.sh`. Smoke-tested first (11.74 min/epoch, cheaper
per-epoch than the FT+token_aggregation baseline's ~10.4 min/epoch scaled from its
known 4h19m/25ep — consistent with the attentive pooler being lighter than a
per-token linear classifier + topk-mean over 4608 tokens); full 3-seed run completed
cleanly (rc=0, 25/25 epochs, all seeds).

| arm | mean±std | ensemble | Δ vs frozen A0 baseline (ensemble) |
|---|---|---|---|
| Frozen, `token_aggregation` (A0) | 31.18±1.64 | 32.36 | — |
| Frozen, attentive-shallow (A1) | 34.66±0.62 | 37.06 | +4.70 |
| FT-last4, `token_aggregation` (Step 0 FT) | 37.68±0.23 | 38.69 | +6.33 |
| **FT-last4, attentive-shallow (combined)** | **35.82±0.64** | **38.03** | **+5.67** |

**The two effects do NOT stack — they interfere, converging to roughly what FT alone
gets.** Combined (FT + A1's head) lands at 38.03 ensemble / 35.82±0.64 mean, slightly
*below* FT with the original `token_aggregation` head (38.69 / 37.68±0.23) — a small
regression, well within these arms' own seed spread, but clearly not additive (naive
sum would predict roughly 32.36 + 4.70 + 6.33 − 32.36 ≈ 43, nowhere close). **Reading:
A1's frozen-arm win is compensating for information the frozen encoder doesn't surface
in a token-aggregation-friendly way; once FT lets the encoder itself relocate
task-relevant signal toward wherever the head reads it, the pooling mechanism stops
mattering much** — a simpler per-token linear head is just as good as (marginally
better than) the fancier attentive pooler once the encoder is doing real work upstream.
**FT with its original head remains the single strongest lever tested in this whole
plan; there is no known way to beat it by also changing the head.**

Related: [[budget-scaling-shows-up-in-finetune]], [[fine-tune-beats-frozen-readout]].

### Follow-up: FT unfreeze-depth sweep — last-4 vs last-8 vs full (triplet, prod37M_e199, 2026-08-23/24)

Direct test of whether unfreezing more than the last 4 blocks buys further IVT, holding
the head fixed at `token_aggregation` (Step 0 FT's original head — the only head that
was NOT beaten by pooling under FT per the subsection above). Same checkpoint, same
data, same LR (`encoder_lr: 1e-5`, held fixed across depths — a real confound for
full-unfreeze's batch-size change, noted below). No config anywhere in this repo had
tried FT-8 or full unfreeze on triplet before this pass; last-4 is the only depth
Step 0 measured.

**Two operational bugs found while setting this up, both durable lessons independent of
the IVT numbers:**

1. **Full unfreeze (48/48 blocks, 1.84B trainable params) OOMs at `batch_size=4`**
   (`RuntimeError: level_zero backend failed with error: 40
   (UR_RESULT_ERROR_OUT_OF_RESOURCES)`, on the very first forward pass) but trains
   cleanly at `batch_size=2`. `LR was held fixed despite the batch-size halving` —
   not rescaled, so full-unfreeze's row below is confounded by both depth AND an
   implicitly higher effective LR-per-example than last-4/last-8's `batch_size=4`.
2. **`mpiexec` returned `rc=0` on the crashed full-unfreeze (bs=4) attempt despite a
   rank exiting with code 1 and another dying to SIGTERM.** This is the SECOND time
   this session that `mpiexec`/PALS masked a real crash behind a clean exit code — the
   first was the GraSP export `WORLD_SIZE` bug (§9 above). **Standing rule for any
   future work in this repo: grep the actual log for `Traceback` / the specific error
   signature before treating ANY run as successful — `rc=0` alone is not evidence.**
   Both new launcher scripts (`scripts/tripft_last8_seed_sweep.sh`,
   `scripts/tripft_full_bs2_seed_sweep.sh`) now bake this check in directly: each
   greps its own per-seed log for `Traceback` after `wait` and overrides the exit
   code to failure if found, regardless of what `mpiexec` reported.

**Result — last-8 (`configs/heads/triplet/finetune/tripft_last8_prod37M_e199.yaml`,
`encoder_unfreeze_last_n: 8`, `batch_size: 4`, same as last-4):**

| arm | mean±std | ensemble | Δ vs last-4 (ensemble) |
|---|---|---|---|
| FT-last4, `token_aggregation` (Step 0 FT baseline) | 37.68±0.23 | 38.69 | — |
| **FT-last8, `token_aggregation`** | **39.92±0.91** | **41.24** | **+2.55** |

Smoke-tested clean first (12.68 min/epoch, zero Tracebacks, checkpoint saved); full
3-seed run completed cleanly (rc=0 AND zero Tracebacks in every seed's log, 25/25
epochs each). Train/val gap at the final epoch is modest and consistent across seeds
(e.g. seed 0: train tool-MicroF1 94.85% vs val 94.55%, train verb-MicroF1 88.15% vs val
83.91%) — no sign of overfitting from the added capacity on triplet's 1704-clip train
set. **Last-8 is a clean, unambiguous win over last-4** — +2.55 ensemble IVT, +2.24
mean IVT, both well outside last-4's own seed spread (±0.23) and last-8's own
(±0.91). **This tracks the same direction as the GraSP precedent** (Leonardo's
`fs_e159` lineage: last-4=83.66 → last-8=85.34 mAP, +1.68) — unfreezing more of the
encoder helps on both a much larger corpus (GraSP) and triplet's small 1704-clip one,
contrary to the a priori overfitting concern that motivated smoke-testing this
carefully before committing compute. **Last-8 + `token_aggregation` is now the best
triplet result found in this entire investigation, surpassing FT-last4 by a clear
margin.**

Full-unfreeze (`tripft_full_prod37M_e199_bs2.yaml`, batch_size=2, LR unchanged) was
still training as of this write-up — its row and the completed 3-way depth verdict
(does IVT keep climbing at 48 blocks, plateau, or reverse) will be appended once it
lands; treat the last-8 result above as the current best independently of what
full-unfreeze shows, since it is fully verified on its own.

**Operational note (2026-08-24):** full-unfreeze's original `capacity`-queue submission
sat queued 8+ hours with "reservation or top job conflict" before it became clear this
was a genuine incoming system maintenance window (not generic congestion) that the
scheduler was correctly refusing to place a long job across. Cancelled the 14h job
(never ran, nothing lost) and switched to chaining 1h chunks across `debug-scaling` and
`capacity` — whichever had a free slot — relying on `resume_checkpoint: true` to carry
state between chunks (each launcher greps its own log for `Traceback` post-`wait` and
overrides a false `rc=0`, so a walltime-killed chunk is verified as genuinely clean
before resubmitting, not just trusted). Two chunks landed **2 full epochs cleanly for
all 3 seeds** (checkpoints at epoch 2, val_micro_f1 82.12/82.16/82.58%, zero Tracebacks)
before the maintenance reservation began actively blocking even 1h submissions
(`qstat` comment changed to explicit "conflict with reservation" on a queued
1h/3-node request) — confirming the maintenance window had arrived.

**Maintenance cleared 2026-08-24 evening.** The queued `debug-scaling` 1h chunk
(job 8778521) placed and resumed cleanly from `latest.pt` past the pre-maintenance
2-epoch checkpoint — by the time of this update it had reached epoch 3 with zero
Tracebacks across all 3 seeds, confirming `resume_checkpoint: true` carried state
correctly across the chunking detour. With the maintenance window gone, reverted to
the original strategy: submitted the standard `tripft_full_bs2_seed_sweep.sh` (14h,
`capacity`, 3 nodes) as job 8778666 with `-W depend=afterany:8778521`, so it starts
the moment the in-flight chunk exits (walltime or completion) and resumes from
whatever `latest.pt` the chunk left behind — no further manual chunking needed to
finish the remaining epochs.

Related: [[budget-scaling-shows-up-in-finetune]], [[aurora-multi-mpi-per-pbs-worldsize]],
[[grasp-probe-noise-floor-3map]].

## 10. GraSP-FT (last-4, prod37M_e199) — two bugs found, training config still on stale CSVs (2026-08-23)

**Bug 1 (checkpoint-loading, FIXED):** `evals/video_classification_frozen/modelcustom/vit_encoder_multiclip_v21.py`
only stripped `module.`/`backbone.` prefixes when loading an encoder checkpoint. Partial-FT
checkpoints (`encoder_unfreeze_last_n>0`) save `state_dict()` off the `ClipAggregation` wrapper,
whose raw ViT lives under `self.model` — every key came back `model.blocks.N....` and none
matched, so the GraSP-FT re-export step (needed to re-score with the tuned encoder) silently left
the entire 48-block backbone at random init while reporting `rc=0`. First symptom: seed0 scored
best-head mAP=29.65/ensemble=31.36 — a collapse, not a regression. Fixed by adding a `model.`
strip (only fires when present, so frozen/`backbone.`-prefixed checkpoints are unaffected); the
fix was verified via `<All keys matched successfully>` in the re-export log for all 3 seeds
(previously: `missing_keys` covered all 48 blocks). Triplet-FT and SAR-FT scoring do NOT hit this
path — both load the FT state directly onto the already-`ClipAggregation`-wrapped encoder object,
so their existing results (SAR-FT ceiling 90.81 F1@10, Triplet-FT ensemble 38.69 IVT mAP) stand
unaffected. Also found and fixed in the same pass: `scripts/grasp_ft_headtrain_prod37m_e199.sh`'s
self-resubmit chain twice hit a **hard PBS walltime kill** (`job killed: walltime ... exceeded
limit`) that raced past the script's own graceful resubmit-decision block — the same failure
class as `[[wallclock-kill-deletes-the-closing-anchor]]`. Required a manual resubmit both times;
checkpoints (`latest.pt`, saved via `resume_checkpoint: true`) were intact and resumed cleanly.

**Post-fix GraSP-FT result, prod37M_e199, 3 seeds (`scripts/eval_grasp_map_cached.py`):**

| seed | best-head mAP | ensemble mAP | top-2 mAP | best epoch |
|---|---:|---:|---:|---|
| s0 | 82.95 | 84.36 | 84.09 | ep10 |
| s1 | 84.27 | 84.52 | 84.67 | ep5 |
| s2 | 83.00 | 83.64 | 83.86 | ep4 |
| **mean** | **83.41** | **84.17** | **84.21** | — |

vs. frozen block-47 baseline (§9, same checkpoint, same protocol): 82.64 best-head / 84.84
ensemble. **GraSP-FT is essentially flat vs. frozen** (+0.77 best-head, −0.67 ensemble) — unlike
Triplet-FT (+6.33 IVT ensemble, §8) and SAR-FT (~+3.4 F1@10), which both showed clean FT gains on
this same checkpoint.

**Bug 2 (config drift, NOT YET FIXED) — the likely reason GraSP-FT looks flat here vs. Leonardo's
FT numbers on an older checkpoint.** User flagged (from memory) that a prior lineage without
openh/kinetics achieved FT-4 ≈ 83.66 and FT-8 ≈ 85.34. Verified via background research agent
against Leonardo's directory (`/flare/ModCon/leonardo_borgioli/probes/`):

- **83.66** = mean of 3 seeds, last-4 unfreeze, checkpoint `fs_e159`
  (`/flare/ModCon/ngetty/checkpoints/surg_2_1_vitG384_fixedshape/vitG384_n16g12_weak/e159.pth.tar`,
  has kinetics400 but NOT openh, confirmed via its saved `params-pretrain.yaml`): seed0=84.87
  (`grmap_phaseS16sh.o8760952`), seed1=84.03, seed2=82.09 → mean 83.6633.
- **85.34** = mean of 3 seeds, **last-8** unfreeze (not full/all-48-block FT — no config in
  either tree unfreezes more than 8 blocks), same `fs_e159` checkpoint: seed0=85.68
  (`probes/runs/grasp_v1/map_pbs_logs/8735698...OU`), seed1=85.32, seed2=85.03 → mean 85.3433.
- Configs: `/flare/ModCon/leonardo_borgioli/probes/configs/grasp_v1/grasp_phase_2B_off16_ft_last{4,8}{,_seed1,_seed2}.yaml`.

**Config diff (Leonardo's `grasp_phase_2B_off16_ft_last4.yaml` vs. our
`configs/heads/grasp/unfreeze_384/prod/grasp_prod37m_e199_last4_s0.yaml`):**

| field | Leonardo's (83.66) | ours (prod37M_e199) |
|---|---|---|
| `dataset_train`/`dataset_val` | `/flare/ModCon/ngetty/data/grasp_ctx_official/csv/...` (corrected leak-free builder) | `/flare/ModCon/leonardo_borgioli/probes/csv/grasp_phase_asformer_ctx3_seq_*.csv` (**stale pre-2026-08-03 ctx3 builder** — 6.6% label drift + pixel shortcut + non-centered windows, per that file's own header) |
| `num_segments` / `num_clips` | 4 | 3 |
| `temporal_tokens` | 32 | 24 |
| `num_views_per_segment` | 1 | 2 |
| `loss_smoothing_weight` | 0.15 | 0.25 |
| `encoder_lr` / `encoder_warmup` | 5e-6 / 0.15 | 1e-5 / 0.05 |

**Our GraSP-FT training config never got migrated to the `grasp_ctx_official` CSVs** — every
other GraSP config in this repo (frozen export/probe, FT re-export/re-score) uses the corrected
builder; only the FT *training* config (`configs/heads/grasp/unfreeze_384/prod/*.yaml`) is still
on the old `ctx3` CSVs Leonardo's own header comment says were superseded. This means our
GraSP-FT training ran on different (worse) data/window-geometry than what it was then scored
against (`grasp_ctx_official` cache, via the export/probe configs) — an apples-to-oranges setup
baked into the config, not a checkpoint-quality signal. **Not yet re-run**: the fix is to clone
`configs/heads/grasp/unfreeze_384/prod/*.yaml` onto Leonardo's geometry/CSVs/hyperparameters
before drawing any conclusion about whether prod37M_e199 (openh+kinetics) underperforms fs_e159
(kinetics only) on GraSP-FT specifically.

**Other results found in Leonardo's directory (not yet cross-checked against our lineage):**
Central doc `/flare/ModCon/leonardo_borgioli/probes/configs/grasp_v1/GRASP_BENCHMARK_STATUS.md`
records: frozen fs_e159 (pre-fix ctx3) best-head 71.01; frozen fs_e159 (post-fix official,
dropout-swept) best-head 79.72; meta2b (raw, uncontinued) frozen off16 ~82.06–82.73, FT-4
~80.83–82.30; last-2 unfreeze ("C1") neutral vs frozen (pre-official-rebuild numbers, not
comparable to the table above); Step-recognition frozen 39.48 → FT-last4 (v2, post encoder-reset
bugfix) 40.00 mAP; ~90 total configs spanning 1B/2B/meta/lemon/surgenet/endovit × frozen/FT-4/FT-8
× 3 seeds under `probes/configs/grasp_v1/`.

Related: [[grasp-eval-leak-fixed]], [[grasp-probe-input-mismatch]], [[wallclock-kill-deletes-the-closing-anchor]],
[[ft-encoder-resume-bug]], [[fine-tune-beats-frozen-readout]], [[budget-scaling-shows-up-in-finetune]].

## 11. GraSP Step head-width / dropout A/B — DEFAULT CHANGED (2026-08-22/24)

**Trigger.** The cached GraSP Step head-train ran at 1.85 s/iter for a batch_size=2
probe. Decomposed via the val loop (forward-only, same shards,
[[val-loop-decomposes-the-iteration]]) and `profile_timing`: **backward+all-reduce is
94-97.5% of the iteration**, not data or head-forward. The head runs its full
O(D^2) body at the ENCODER's width (`embed_dim=1664` for ViT-g) because
`asformer_kwargs.head_embed_dim` was unset — with the 3-head `multihead_kwargs` LR
sweep, that's **466M params = 1.86 GB of fp32 gradients all-reduced every iteration**
to train a probe on 9,191 samples. See [[grasp-head-width-is-the-probe-speed-lever]]
for the full derivation.

**A/B: 3 arms x 3 seeds, all on prod37M_e199 Step (21-cls), same LR sweep, same 20-epoch
cap / patience-6 early stop.** `head_embed_dim` projects the encoder's output down to a
narrower working width at head entry (already wired, `ASFormerHead(input_dim=encoder.
embed_dim, embed_dim=head_embed_dim)`), so the whole ASFormer body — spatial pool,
temporal self-attention, temporal blocks — runs at the narrower width, not just the
final classifier.

| arm | seed0 | seed1 | seed2 | mean val_macro_f1 | std |
|---|---|---|---|---|---|
| d1664_dr00 (baseline: full-width, dropout 0) | 52.053% (ep4) | 52.348% (ep17) | 51.649% (ep12) | 52.02% | 0.29% |
| d768_dr00 (narrow head, dropout 0) | 52.294% (ep12) | 53.387% (ep14) | 52.985% (ep14) | 52.89% | 0.45% |
| d768_dr03 (narrow head, dropout 0.3) | 53.743% (ep17) | 53.486% (ep13) | 53.467% (ep17) | **53.57%** | **0.14%** |

**Reading.** d768_dr00 beats d1664_dr00 by +0.87pp — narrowing the head is not just a
speed win, it recovers val performance the wide head was losing to overfitting: the
d1664 arms hit their best epoch far earlier on average (ep4/17/12) than d768 (ep12/14/14),
and the original 20-epoch d1664 baseline run (pre-A/B) showed train_acc 97.3% vs
val_acc 60.3% with val_loss climbing monotonically — textbook overfitting on a
155M-param head. Adding dropout 0.3 on top of the narrow head adds another +0.68pp
AND has the tightest seed spread of any arm tested (std 0.14% vs 0.29-0.45%) —
narrower + regularized is both better and more reproducible, not a noisy tradeoff.

Total gain d1664_dr00 -> d768_dr03: **+1.55pp mean val_macro_f1, at ~3.7x lower
per-iteration compute** (0.5s vs 1.85s/it, both backward-bound per the same profiling).
Per-epoch: ~12.5 min -> ~3.4 min; a 20-epoch run collapses from ~4.2h to ~1.1h, i.e.
~5 one-hour `debug`-queue walltime slices down to ~1.5 (still needed 2 resume cycles
per seed in practice due to queue contention, not a step-count problem).

**mAP scoring, all 9 arms (`eval_grasp_map_cached.py`, fanned out 1 arm/node via
`scripts/grasp_step_ab_score_fanout.sh`, ~10 min total for all 9):**

| arm | ensemble mAP (mean±std) | top-2 mAP (mean±std) | best-head mAP (mean±std) |
|---|---|---|---|
| d1664_dr00 (baseline) | 57.12±0.31 | 56.26±0.78 | 54.04±0.61 |
| d768_dr00 (narrow, no dropout) | **57.85±0.55** | **56.89±0.43** | **54.46±0.77** |
| d768_dr03 (narrow + dropout 0.3) | 56.32±0.52 | 55.92±0.85 | 54.20±1.16 |

**★ mAP CONTRADICTS val_macro_f1 on which arm wins — d768_dr03 does NOT ship.**
val_macro_f1 (the head-train's own logged metric, table above) ranked
dr03 (53.57%) > dr00 (52.89%) > d1664 (52.02%), with dr03 also having the tightest
seed spread (std 0.14%). The mAP scoring pass (the actual paper metric, comparable to
TAPIS/Leonardo) inverts the top two: **d768_dr00 wins on every mAP variant**, and
**d768_dr03 falls BELOW the d1664 baseline on ensemble mAP** (56.32 vs 57.12) while
having the WIDEST seed spread of any arm on best-head mAP (std 1.16 vs
0.61-0.77) — the opposite of its val_macro_f1 behavior. Plausible mechanism: dropout
0.3 improves macro-F1's per-class precision/recall balance at a fixed decision
threshold, while hurting AP's confidence-ranking of predictions (AP is threshold-free
and sensitive to how sharply-peaked the score distribution is; dropout flattens it).
**Lesson: val_macro_f1 from a head-train log is not a safe proxy for the paper metric
when picking a default — score mAP before shipping, not after.** An initial ship of
d768_dr03 based on val_macro_f1 alone was reverted to d768_dr00 within the same day
once this scoring pass came back.

d768_dr00 vs d1664_dr00 agrees in direction on BOTH metrics (val_macro_f1 +0.87pp;
ensemble mAP +0.73, 57.85 vs 57.12) — this is the arm actually shipped.

**★ CORRECTION 2026-08-24: a prior draft of this section cited a Leonardo Step
anchor of "59.69 mAP" — that number does not exist anywhere in Leonardo's own
`GRASP_BENCHMARK_STATUS.md` and was never verified against his file before being
written down. Verified against the source doc just now:**

| source | checkpoint | metric | Step mAP |
|---|---|---|---|
| TAPIS paper Table 4 (older) | — | test | 49.42 |
| **TAPIS repo README (canonical, per Leonardo's doc)** | — | test | **52.01** |
| Leonardo, frozen 2B probe | `fs_e159` (1B-lineage CPT, NOT raw meta2b) | test, single seed, best-of-3-LR-heads | **39.48** |
| Leonardo, FT-last4 | `fs_e159` | test | 38.67 — **INVALIDATED by Leonardo** (encoder-reset-on-resume bug, his own doc flags this) |
| **This A/B, d768_dr00 (shipped default)** | `prod37M_e199` | test, 3-seed mean | **57.85 ± 0.55 ensemble** |

d768_dr00 beats the canonical TAPIS SOTA (52.01) by +5.84 ensemble mAP, and beats
Leonardo's own frozen fs_e159 result (39.48) by a wide margin — there is no
regression here once the fabricated 59.69 figure is removed. **Do not cite the
59.69 number anywhere; it was an error, not a measurement.**

**Default changed 2026-08-24 (corrected same-day after mAP scoring).**
`configs/heads/grasp/official_ctx16/grasp_off16_step_prod37M_e199_probe.yaml` now sets
`head_embed_dim: 768`, `dropout: 0.0` (was unset/0.0 at d1664 width), with `folder`/`tag`
retargeted to a `_d768dr00` suffix so `resume_checkpoint: true` cannot load the old
d1664-shaped checkpoint into the new narrower head (the old d1664 checkpoint at the
original path is left in place, matching the "best-head 54.04 / ensemble 57.12" scoring
above). The A/B's own per-arm configs (`configs/heads/grasp/official_ctx16/_ablate/
ab_{d1664_dr00,d768_dr00,d768_dr03}_s{0,1,2}.yaml`), training runner
(`scripts/grasp_step_headwidth_ab.sh`), and scoring fanout
(`scripts/grasp_step_ab_score_fanout.sh`) are left in the repo for reuse (e.g.
re-running this A/B on GraSP Phase, which shares the same head architecture and is a
stated open item — see [[grasp-head-width-is-the-probe-speed-lever]]'s anchor caveat:
Phase's ledger numbers 75.65/79.77 were produced at d1664 and are not comparable to a
d768 rerun without re-running the d1664 anchor alongside).

**Data provenance verified clean for this A/B**: `dataset_train`/`dataset_val` CSVs
(`grasp_ctx_official/csv/grasp_step_asformer_ctx_seq_*.csv`, dated 2026-08-22) and the
feature cache manifests (dated 2026-08-22, postdating the CSVs) both come from the
fixed `build_grasp_ctx_official.py`, not Leo's fps-drift-buggy `ctx3` builder
([[probe-builder-fps-audit-2026-08-13]]) — confirmed by filename pattern
(`ctx_seq` vs buggy `ctx3_seq`), directory (`grasp_ctx_official` vs Leo's original
`leonardo_borgioli/probes/csv/`), and file dates (Aug 22 vs Leo's untouched Jul 8).

Related: [[grasp-head-width-is-the-probe-speed-lever]], [[val-loop-decomposes-the-iteration]],
[[grasp-probe-noise-floor-3map]], [[probe-stability-dropout]], [[grasp-step-task-gap-diagnosis]],
[[aurora-userSite-torch-shadow]] (the scoring pipeline's XPU-vs-CPU bug hit during this work).

## 11b. GraSP Step head-width/dropout ablation — val_macro_f1 and mAP DISAGREE on the winner (2026-08-24)

_(Duplicate-numbered with §11 above by two concurrent writers; both settled on the same
verdict — d768_dr00 ships, mAP overrules val_macro_f1 — kept separate since this entry has
the full per-arm/per-seed breakdown table §11 summarizes.)_

3-seed A/B on GraSP Step (frozen probe, prod37M_e199, 21 classes), varying ASFormer
`head_embed_dim` (1664 default vs 768 narrow) and `dropout` (0.0 vs 0.3). Two
different metrics were used at two different points and they **rank the arms
differently** — this section reconciles them; **mAP is the metric of record**
(matches TAPIS's paper metric and every other cross-checkpoint table in this
ledger), so it settles the choice.

**val_macro_f1** (training-loop selection metric, 3 seeds each):

| arm | seed0 | seed1 | seed2 | mean | std |
|---|---:|---:|---:|---:|---:|
| d1664_dr00 (baseline) | 52.053% | 52.348% | 51.649% | 52.02% | 0.29% |
| d768_dr00 (narrow) | 52.294% | 53.387% | 52.985% | 52.89% | 0.45% |
| d768_dr03 (narrow+dropout) | 53.743% | 53.486% | 53.467% | 53.57% | 0.14% |

Ranking by val_macro_f1: **d768_dr03 > d768_dr00 > d1664_dr00**.

**mAP** (`scripts/eval_grasp_map_cached.py`, GraSP Step TEST split, job 8777706, same
3 seeds/arm, best-head and mean-of-3-heads ensemble):

| arm | best-head mean±std | ensemble mean±std |
|---|---:|---:|
| d1664_dr00 (baseline) | 54.04 ± 0.50 | 57.12 ± 0.25 |
| **d768_dr00 (narrow)** | **54.46 ± 0.63** | **57.85 ± 0.45** |
| d768_dr03 (narrow+dropout) | 54.20 ± 0.95 | 56.32 ± 0.43 |

(std here is population std, n=3; §11's table above uses sample std, n-1 —
means are identical between the two tables, only the std convention differs.
Ranking and every qualitative claim below are unaffected either way.)

Ranking by mAP: **d768_dr00 > d768_dr03 ≈ d1664_dr00** (dr03 is actually the
*worst* arm on ensemble mAP, and has the widest best-head spread of the three,
0.95 vs 0.50–0.63).

**Reading — the two metrics disagree on dropout, agree on width:**
- **Narrowing the head (1664→768) is a clean win on both metrics** — d768_dr00
  beats d1664_dr00 on val_macro_f1 (+0.87pp), best-head mAP (+0.42), and
  ensemble mAP (+0.73), at ~3.7x lower compute per
  [[grasp-head-width-is-the-probe-speed-lever]]. Not in dispute.
- **Adding dropout 0.3 helps val_macro_f1 (+0.68pp over d768_dr00) but HURTS
  mAP** (best-head −0.26, ensemble −1.53, worst of all three arms) **and widens
  the mAP seed spread** (0.95 vs 0.63) despite tightening the val_macro_f1 spread
  (0.14% vs 0.45%). Plausible mechanism: dropout smooths the confident
  high-probability predictions that dominate AP's ranking (a few very-confident
  correct predictions carry a class's AP), while it helps macro-F1's
  precision/recall balance at a fixed threshold — the two metrics reward
  different things from the same regularization.
- **Do not select head configuration from val_macro_f1 alone on this probe** —
  it already had this exact failure mode with FT epoch selection
  ([[grasp-step-task-gap-diagnosis]] and the FT-vs-frozen mAP/val-F1 divergence
  in section 3's Stage C ladder); this is the same divergence appearing one
  level up, in head-architecture selection instead of epoch selection.

**Decision: ship d768_dr00 (narrow head, no dropout) as the new default GraSP
head architecture** — the width win is metric-agnostic and free (3.7x speedup),
the dropout addition is a net mAP loss despite looking like a win on the proxy
metric. Do NOT carry d768_dr03 forward into FT runs or the Phase transfer test.

Related: [[grasp-head-width-is-the-probe-speed-lever]], [[probe-stability-dropout]]
(dropout helped *stability* on a different probe/checkpoint — SAR-cached-384 — so
this is not a blanket "dropout never helps" finding, just a per-probe, per-metric
one), [[val-loop-decomposes-the-iteration]].

## 11c. GraSP Phase transfer test of the Step head-width winner (d768_dr00) — NULL (2026-08-24)

Tested whether §11/§11b's Step-task win from narrowing the ASFormer head
(`head_embed_dim` 1664→768, `dropout: 0.0`) transfers to the Phase task (11
classes, same `prod37M_e199` encoder, frozen probe, reusing the existing Phase
frozen feature cache — head width is head-side-only, so no re-export was
needed). 3 seeds, scored via `scripts/eval_grasp_map_cached.py` against
`configs/heads/grasp/official_ctx16/_ablate/phase_d768dr00_s{0,1,2}.yaml`
(jobs 8778441 s0, 8778463 s1, 8778736 s2).

| seed | best-head mAP | ensemble mAP |
|---|---:|---:|
| s0 | 81.27 (ep8) | 82.40 |
| s1 | 83.77 (ep7) | 83.62 |
| s2 | 82.71 (ep3) | 83.72 |
| **mean ± std** | **82.58 ± 1.25** | **83.25 ± 0.73** |

vs. the frozen default-head (d1664_dr00) Phase baseline (§9, block 47):
**82.64 best-head / 84.84 ensemble**.

**Verdict: NULL — does not transfer, and mildly reverses on ensemble.**
Best-head is a wash (82.58 vs 82.64, well inside the ~3 mAP single-seed noise
floor, [[grasp-probe-noise-floor-3map]]). Ensemble is *worse* by −1.59
(83.25 vs 84.84) — outside that noise floor and consistent in direction across
all 3 seeds (every d768dr00 ensemble score sits 1.1–2.4 below the 84.84
anchor). The Step-task win (§11b: +0.42 best-head / +0.73 ensemble) is
task-specific, not encoder- or checkpoint-general: Step is a 21-class
fine-grained action task where narrowing the head's capacity apparently acts
as useful regularization on top of the frozen encoder's token features, while
Phase (11 coarse classes, more per-class training signal) has enough data
that the 1664-wide head's ensemble diversity is worth more than the extra
regularization from narrowing it.

**Decision: keep d1664_dr00 (default width) for Phase; d768_dr00 stays
Step-only.** Do not carry the narrow-head win across tasks by default —
head-width/dropout choices are per-task and must be re-validated when
transferred, same lesson as [[val-loop-decomposes-the-iteration]]'s selection-metric
warning but one level up (architecture choice, not epoch/metric choice).

Related: [[grasp-head-width-is-the-probe-speed-lever]], [[grasp-probe-noise-floor-3map]].
