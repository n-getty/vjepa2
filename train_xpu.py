...
def main():
    parser = argparse.ArgumentParser(description='V-JEPA Droid Training on XPU')
    ...
    # The rest of the training logic will go here...
    import copy
    import gc
    ...
    target_encoder = DistributedDataParallel(target_encoder, device_ids=[local_rank])
    ...
    if skip_batches > 0:
        ...
        for itr in range(skip_batches):
            if master_process and (itr % 10 == 0):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for itr in range(ipe):
            ...
            def train_step():
                ...
                # Step 1. Forward
                with torch.autocast(device_type='xpu', dtype=dtype, enabled=mixed_precision):
            ...
            (
                ...
                _new_lr,
                _new_wd,
            ), gpu_etime_ms = gpu_timer(train_step, device_type='xpu')
            ...
            def log_stats():
                ...
                if master_process and ((itr % log_freq == 0) or (itr == ipe - 1) or np.isnan(loss) or np.isinf(loss)):
                    logger.info(
                        "[%d, %5d] loss: %.3f [%.2f, %.2f] "
                        "[wd: %.2e] [lr: %.2e] "
                        "[mem: %.2e] "
                        ...
                        % (
                            ...
                            _new_wd,
                            _new_lr,
                            torch.xpu.max_memory_allocated() / 1024.0**2,
                            iter_time_meter.avg,
                            gpu_time_meter.avg,
