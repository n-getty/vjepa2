#!/usr/bin/env bash
# 16-NODE HSDP + grad_accum=2 — the FABRIC-CONTENTION MITIGATION run (per §4e verdict).
#
...
CKPT_DIR=/flare/ModCon/ngetty/checkpoints/SPIKETEST_vitG384/ga2_n16g12
...
# -------- FABRIC-CONTENTION LEVER (§4e verdict) --------
# The env-diff (job 8643398) proved the residual is fabric contention (cohort spikes +
# FLAT l0-free/l0-ext), not memory accumulation. grad_accum=2 halves inter-node AllReduce
# frequency — PRISM's accepted mitigation, verified end-to-end at 1n (job 8643426: 40 iters
# clean, weight_distance_loss squeeze(1) path OK, no NaN). bs=2 -> micro-bs=1; the trainer's
# accum path already calls encoder/predictor .no_sync() on the non-final microbatch
# (train.py:1017) = PRISM's FSDP_NO_SYNC_ACCUM behavior, so this env is the only knob.
# PASS = cohort spikes flatten / p50 backward stops escalating over ipe200.
