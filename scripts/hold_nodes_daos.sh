...
#PBS -N holddaos
#PBS -A AuroraGPT
# debug-scaling. MEASURED at 2 nodes (job 8731170 run_1): 9m22s end to end --
# ~6m40s startup (rendezvous + 28 GB DAOS ckpt load + 2B build) and 2m40s for 6
# iterations, of which iteration 0 alone is 120 s. Three serialized arms are
# therefore ~30 min, which fits the 1 h cap with room to spare.
#
# capacity is the WRONG home despite its 7-day walltime: max_run is 2 per user
# and both running slots are normally the live training arms, so a hold there
# queues behind them indefinitely.
#PBS -q debug-scaling
#PBS -l select=2
