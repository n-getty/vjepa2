...
ALLOWED_GPUS_PER_NODE = {2, 4, 8}
BASE_GPU_COUNT = 4  # baseline single-node config the YAMLs were authored for
...
def topology_suffix(num_gpus: int, num_nodes: int) -> str:
...
def scale_local_batch(base_batch: int, total_gpus: int, strong_scale: bool) -> int:
    if strong_scale:
        return base_batch
...
def remap_folder(folder: str, num_gpus: int, num_nodes: int) -> str:
    suffix = topology_suffix(num_gpus, num_nodes)
    return f"{folder}{suffix}" if suffix else folder


def remap_checkpoint_path(path_value: str | None, num_gpus: int, num_nodes: int) -> str | None:
    suffix = topology_suffix(num_gpus, num_nodes)
...
def remap_repo_checkpoint_path(
    path_value: str | None,
    num_gpus: int,
    num_nodes: int,
    repo_root: Path,
    ...
    return remap_checkpoint_path(str(raw_path), num_gpus, num_nodes)
...
def main():
    ...
    cfg["folder"] = remap_folder(base_folder, args.num_gpus, args.num_nodes)
    ...
    data_cfg["batch_size"] = scale_local_batch(base_batch, total_gpus, args.strong_scale)
    cfg["data"] = data_cfg

    meta_cfg = cfg.get("meta", {})
    meta_cfg["pretrain_checkpoint"] = remap_repo_checkpoint_path(
        meta_cfg.get("pretrain_checkpoint"), args.num_gpus, args.num_nodes, repo_root
    )
    meta_cfg["read_checkpoint"] = remap_repo_checkpoint_path(
        meta_cfg.get("read_checkpoint"), args.num_gpus, args.num_nodes, repo_root
    )
    cfg["meta"] = meta_cfg

    optimization_cfg = cfg.get("optimization", {})
    optimization_cfg["anneal_ckpt"] = remap_repo_checkpoint_path(
        optimization_cfg.get("anneal_ckpt"), args.num_gpus, args.num_nodes, repo_root
    )
