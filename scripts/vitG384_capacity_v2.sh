#!/usr/bin/env bash
# _v2 branch of vitG384_capacity.sh for the next 2B CPT run (2026-07-20).
# Changes vs vitG384_capacity.sh: BASE_CFG/RUNTIME_CFG -> vitG384_fixedshape_v2.yaml
# (kinetics400->pe_video, grasp->grasp_noleak; see that config's header for detail),
# CKPT_DIR -> a fresh surg_2_1_vitG384_v2 directory (NOT resumed from the old
# fixedshape e215 checkpoint -- that checkpoint's weights already saw the GraSP
# leak and a fresh Meta-init keeps any LR change directly comparable to the
# original launch conditions), jobname vitGc2 (short, avoids qstat -u jobname truncation past 9 chars; so qstat/resubmit greps dont
# collide with a still-running v1 job). All hardening (watchdog, hang forensics,
# resubmit policy, HSDP env, staging) is UNCHANGED from the proven v1 launcher.
#
...
# ARM OVERRIDES (default = the proven v2 run; unset means nothing changes).
# Pass with `qsub -v VJEPA_CFG_NAME=vitG384_lbA,VJEPA_CKPT_DIR=...,VJEPA_TRUE_ACCUM=16`
# to reuse this launcher's proven env/staging/watchdog for a different config
# without forking the script. VJEPA_TRUE_ACCUM is already read further down.
CFG_NAME=${VJEPA_CFG_NAME:-vitG384_fixedshape_v2}
BASE_CFG=$ROOT/configs/vitg16_surg_vid_webdataset_single4/${CFG_NAME}.yaml
RUNTIME_CFG=$ROOT/.runtime_configs/n16g12_weak/configs/vitg16_surg_vid_webdataset_single4/${CFG_NAME}.yaml
PY=/opt/aurora/26.26.0/frameworks/aurora_frameworks-2025.3.1/bin/python
CKPT_DIR=${VJEPA_CKPT_DIR:-/flare/ModCon/ngetty/checkpoints/surg_2_1_vitG384_v2/vitG384_v2_n16g12_weak}
if [[ ! -f "$BASE_CFG" ]]; then
  echo "FATAL: config not found: $BASE_CFG (VJEPA_CFG_NAME=$CFG_NAME)"; exit 1
fi
...
JOB_START_S=$(date +%s)
# Walltime as requested above (#PBS -l walltime=04:00:00), in seconds, so the
# soft-deadline watchdog below can compute a margin without hardcoding it twice.
WALLTIME_S=$((4*3600))
...
#
# WAIT-FOR-LOCK (added 2026-07-22, after job 8685381: a capacity slot that finally
# came through after ~5.5h queued instantly SKIPPED because a debug-scaling chain
# slice happened to be mid-flight, burning the entire scarce 16n capacity allocation
# for ~14s of work -- worst case outcome, the exact opposite of what the chain exists
# for (fill time WHILE waiting for capacity, not compete with it once it lands).
# Chain slices are short (~20-30min including staging, ipe=30) — poll and WAIT for
# one to finish rather than giving up on the first check. Bounded at 40min so a
# genuinely stuck/looping chain doesn't burn the whole 4h capacity walltime on
# waiting alone (this job's 4h budget easily absorbs a 40min wait either way).
LOCK_WAIT_DEADLINE_S=2400
_lock_wait_start=$(date +%s)
while [[ -f "$LOCK" ]]; do
  HOLDER=$(cat "$LOCK" 2>/dev/null || echo "")
  if [[ -z "$HOLDER" ]] || ! qstat "$HOLDER" 2>/dev/null | awk 'NR>2{print $5}' | grep -q '^R$'; then
    echo "stale lock from $HOLDER (not running) — taking over."
    break
  fi
  _waited=$(( $(date +%s) - _lock_wait_start ))
  if (( _waited > LOCK_WAIT_DEADLINE_S )); then
    echo "LOCK held by running job $HOLDER for >${LOCK_WAIT_DEADLINE_S}s — giving up (something is likely stuck; investigate the chain). Skipping."
    echo "JOB END (skipped): $(date)"
    exit 0
  fi
  echo "LOCK held by running job $HOLDER — another slice is training. Waiting (${_waited}s so far, deadline ${LOCK_WAIT_DEADLINE_S}s)..."
  sleep 60
done
echo "$PBS_JOBID" > "$LOCK"
trap 'rm -f "$LOCK"' EXIT
...
# ---- SOFT-DEADLINE WATCHDOG (added 2026-07-21, after job 8681236: a HEALTHY 4h run
# (steady loss, no hang/bad-node) still lost its resubmit — PBS's hard walltime kill
# (14454s vs the 14400s limit) tears down the whole job tree, including this script,
# BEFORE it ever reaches the post-mpiexec resubmit-policy code below. A run that
# survives the full walltime (the case the header comment said "never happens") must
# still hand off cleanly. Stop mpiexec ourselves with a safety margin so the script's
# own shutdown/checkpoint-check/resubmit logic always gets to run, rather than racing
# PBS's SIGKILL.
# MARGIN WIDENED 300->600 (2026-07-21 night, job 8682327): a 300s margin was NOT
# enough — the post-JOB-END resubmit-policy code did a SECOND full torch.load() of
# the 15GB latest.pth.tar (redundant re-read just to get the epoch number) plus a
# grep over the entire (68k+ line) job log for bad-node detection; combined these
# took longer than the remaining 342s, so PBS's hard kill (14458s) still won and the
# chain silently broke (training sat idle ~2.5h before a human noticed and manually
# resubmitted). Fixed BOTH sides: this margin is now 600s, AND the two slow ops below
# were replaced with cheap equivalents (CSV tail instead of torch.load; log tail
# instead of whole-file grep) so the actual post-mpiexec work is now sub-second.
SOFT_DEADLINE_MARGIN_S=600
(
    while true; do
        sleep 60
        pgrep -f "app.main_dist_aurora" >/dev/null 2>&1 || exit 0
        now=$(date +%s)
        elapsed=$((now - JOB_START_S))
        if (( elapsed > WALLTIME_S - SOFT_DEADLINE_MARGIN_S )); then
            echo "SOFT-DEADLINE WATCHDOG: ${elapsed}s elapsed, within ${SOFT_DEADLINE_MARGIN_S}s of the ${WALLTIME_S}s walltime — stopping training gracefully so resubmit logic can run." >&2
            pkill -TERM -f "app.main_dist_aurora"
            sleep 30
            pkill -9 -f "app.main_dist_aurora" 2>/dev/null
            exit 0
        fi
    done
) &
...
kill $SOFT_DEADLINE_PID 2>/dev/null
...
if [[ -n "$_thislog" ]]; then
  ...
  # tail (not full-file grep) -- this log can be 60k+ lines by the time mpiexec exits,
  # and a signal-9 death (if any) always appears right at mpiexec's own exit banner,
  # not buried mid-log, so the tail is sufficient and this is now a soft-deadline-safe op.
  _sig9hosts=$(tail -n 2000 "$_thislog" 2>/dev/null | grep -B1 'died from signal 9' | grep -oE 'x4[0-9a-z]+\.hsn\.cm\.aurora\.alcf\.anl\.gov' | sort -u)
...
# CUR_EP via the CSV's last row (col1 = epoch+1, same value save_checkpoint() writes into
# latest.pth.tar -- see train.py's csv_logger.log(epoch+1, ...) / save_checkpoint(epoch+1, ...))
# rather than a SECOND full torch.load() of the 15GB checkpoint (redundant with the CURRENT_EPOCH
# read at job start, and slow: this used to run in the soft-deadline margin window and blew past
# it, see the SOFT-DEADLINE WATCHDOG comment above -- job 8682327 silently broke the chain).
CUR_EP=$(tail -n 1 "$CKPT_DIR/log_r0.csv" 2>/dev/null | awk -F, '{print $1}')
[[ "$CUR_EP" =~ ^[0-9]+$ ]] || CUR_EP=0
# Known edge case: if killed mid-epoch, the CSV's last row reflects the epoch
# CURRENTLY being processed (may be incomplete), one ahead of the epoch actually
# saved in latest.pth.tar (save_checkpoint runs only after all ipe iters finish).
# Only matters near NUM_EPOCHS (could declare "complete" one epoch early) --
# acceptable given the alternative (torch.load reload) is what broke the chain.
...
if (( CUR_EP >= NUM_EPOCHS )); then
  ...
  # Count only jobs of THIS arm. The stock run is -N vitGc2; arm runs override
  # -N, so grepping the hardcoded vitGc2 would miss an arm's own successor and
  # be confused by the stock run's. Derive the tag from VJEPA_JOBTAG (set
  # alongside -N when submitting an arm) rather than PBS_JOBNAME, which is not
  # guaranteed to be exported into the job environment here.
  JOBTAG=${VJEPA_JOBTAG:-vitGc2}
  QUEUED=$(qstat -u "$USER" 2>/dev/null | grep -c "$JOBTAG")
  if (( QUEUED > 1 )); then
    echo "successor already queued ($QUEUED $JOBTAG jobs) — no resubmit."
  else
    release_lock  # let the successor take the lock cleanly
    # PROPAGATE THE ARM ENV. A bare qsub would drop VJEPA_CFG_NAME/VJEPA_CKPT_DIR/
    # VJEPA_TRUE_ACCUM, so a successor of an arm run would resume the ARM's
    # checkpoint dir using the DEFAULT config and accum -- silently training a
    # different recipe into the same directory. Empty for a stock run.
    RESUB_V=""
    [[ -n "${VJEPA_CFG_NAME:-}" ]] && RESUB_V="VJEPA_CFG_NAME=$VJEPA_CFG_NAME"
    [[ -n "${VJEPA_CKPT_DIR:-}" ]] && RESUB_V="${RESUB_V:+$RESUB_V,}VJEPA_CKPT_DIR=$VJEPA_CKPT_DIR"
    [[ -n "${VJEPA_TRUE_ACCUM:-}" && "${VJEPA_TRUE_ACCUM}" != "1" ]] && \
      RESUB_V="${RESUB_V:+$RESUB_V,}VJEPA_TRUE_ACCUM=$VJEPA_TRUE_ACCUM"
    [[ -n "${VJEPA_JOBTAG:-}" ]] && RESUB_V="${RESUB_V:+$RESUB_V,}VJEPA_JOBTAG=$VJEPA_JOBTAG"
    if [[ -n "$RESUB_V" ]]; then
      # -N too: the in-script `#PBS -N vitGc2` would otherwise rename an arm's
      # successor to the stock jobname, colliding with the real stock run's
      # queued-successor guard. CLI -N overrides the directive.
      NEXT=$(qsub ${VJEPA_JOBTAG:+-N "$VJEPA_JOBTAG"} -v "$RESUB_V" \
        "$ROOT/scripts/vitG384_capacity_v2.sh" 2>&1) \
        && echo "RESUBMITTED successor: $NEXT (env: $RESUB_V)" || echo "resubmit failed: $NEXT"
    else
      NEXT=$(qsub "$ROOT/scripts/vitG384_capacity_v2.sh" 2>&1) && echo "RESUBMITTED successor: $NEXT" || echo "resubmit failed: $NEXT"
    fi
  fi
fi
