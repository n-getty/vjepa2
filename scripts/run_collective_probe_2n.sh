...
export MASTER_ADDR=$(head -n1 $PBS_NODEFILE) MASTER_PORT=29700 WORLD_SIZE=24 LOCAL_WORLD_SIZE=12
echo "=== collective probe: default AR -> mesh -> shard AR -> REPLICATE AR ==="
timeout 300 mpiexec --pmi=pmix -n 24 -ppn 12 --cpu-bind depth --depth 16 \
    python $ROOT/scripts/collective_probe.py 2>&1 \
  | grep -viE "UserWarning|warnings.warn|comm_dev_uuids|CCL_WARN" | grep -iE "r0 |r1 |PASSED|Error|Traceback|all_reduce|mesh|subgroup|init" | tail -30
