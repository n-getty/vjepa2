...
CKPT_DIR=/flare/ModCon/ngetty/checkpoints/SPIKETEST_vitG384/pt213_nohook_n8g12
...
$PY_STAGE $ROOT/scripts/prepare_runtime_config.py \
    $BASE_CFG --root $ROOT --num-gpus 12 --num-nodes 8 --weak-scale > /dev/null
...
mpiexec -n 8 -ppn 1 --cpu-bind none \
    python $ROOT/scripts/stage_node_shards.py \
        ...
        --num-nodes 8 --local-world-size 12 --workers 8
echo "--- staging complete ---"

mpiexec --pmi=pmix -n 96 -ppn 12 --cpu-bind depth --depth 16 \
