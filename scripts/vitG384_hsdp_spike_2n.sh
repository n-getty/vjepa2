...
CKPT_DIR=/flare/ModCon/ngetty/checkpoints/SPIKETEST_vitG384/hsdp_n2g12
RUNTIME_CFG=$ROOT/.runtime_configs/n2g12_weak/configs/vitg16_surg_vid_webdataset_single4/vitG384_cleandata.yaml
...
$PY_STAGE $ROOT/scripts/prepare_runtime_config.py \
    $BASE_CFG --root $ROOT --num-gpus 12 --num-nodes 2 --weak-scale > /dev/null
...
FIRST_ITER_DEADLINE=420   # 10 min: staging+wrap+load+first iter must land by here
