...
#
# Continues the v1 (no-warmup) production run: phase2_main_n16g12_weak.
# Use when the capacity queue is backed up — debug-scaling backfills faster.
#
# debug-scaling is max_run=1/user, so only ONE of these runs at a time.
# The capacity chain (phase2_chain_8h_capacity.sh) targets the SAME ckpt dir,
# so a LOCK GUARD prevents both training v1 concurrently (would corrupt
# latest.pth.tar). Whichever job holds the lock trains; the other skips
# training but keeps its own chain alive by resubmitting.
#
# Submit first instance with:
#   qsub /lus/flare/projects/ModCon/ngetty/vjepa2/scripts/phase2_chain_debugscaling.sh
#
#PBS -N phase2_ds_chain
#PBS -A AuroraGPT
#PBS -q debug-scaling
#PBS -l select=16
...
SELF=$ROOT/scripts/phase2_chain_debugscaling.sh
